<template>
  <div class="main">
    <navBar :pageTitle="pageTitle ? pageTitle : '血糖监测'" backUrl="/monitor/index" />
    ...
    <div style="display: flex; justify-content: center; position: fixed; bottom: 20px; width: 100%;">
      <van-button type="primary" style="width: 90%; background: #67E0A7; border: none"  round @click="$router.push('/monitor/glucoseEditor')">添加记录</van-button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getInfo() {
      ...
      this.loading = true
      Api.findSugarPage(params).then((res) => {
        if (res.data.code === 0) {
          console.log(res, '=====================<')
    ...
    getEchatData() {
      ...
      Api.loadMyBloodSugarTrendData(params).then((res) => {
        if (res.data.code === 0) {
