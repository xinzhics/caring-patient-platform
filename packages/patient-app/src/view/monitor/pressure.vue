<template>
  <div class="main">
    <navBar :pageTitle="pageTitle !== undefined ? pageTitle : '血压监测'" backUrl="/monitor/index"/>
...
export default {
  ...
  mounted() {
    this.getInfo()
    //获取标题
    Api.formOrFormResult(null, 1, null).then(res => {
      if (res.data.code === 0) {
  ...
  methods: {
    ...
    loadMyBloodPressureTrendData() {
      Api.loadMyBloodPressureTrendData({patientId: localStorage.getItem('userId')}).then((res) => {
        if (res.data.code === 0) {
