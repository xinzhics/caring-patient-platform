<template>
  <section style="background:#fff;line-height: 1.1875rem /* 19/16 */">
    <navBar :pageTitle="pageTitle ? pageTitle : '评分结果'"/>
    <div class="content">
      <div class="box" v-if="scoreRule && scoreRule.showResultSum === 1">
      ...
      <div class="box" v-if="scoreRule && scoreRule.showAverage === 1">
        <div class="label"><span>平均分</span></div>
        <div class="value">{{data.formResultAverageScore}}分</div>
      </div>
      <div v-if="scoreRule && scoreRule.showGroupSum === 1">
...
export default {
  ...
  methods: {
    ...
    getFormResultScore() {
      Content.getFormResultScore(this.dataId)
        .then(res => {
          if (res.data.data) {
