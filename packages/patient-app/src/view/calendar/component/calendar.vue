  ...
  <div style="position: relative">
    <div class="my-calendar">
                ...
                <div class="item">
                  <div>
                    <div
                      ...
                      :style="{background: day.date === weekCurrentDate ? '#40BD54' : ''}">
      ...
      <div style="display: flex; justify-content: center; align-items: center; margin-top: 15px; margin-bottom: 15px">
        <van-button type="primary" size="small" style="width: 100px; font-weight: bold" round>打卡情况</van-button>
      </div>
              ...
              <div class="van-step__title">
                ...
                <div v-for="(drug, key) in item.patientDrugsTimes" :key="key"
                  ...
                  <div v-if="drug.status === 2 && compareTime(drug.drugsTime)"
                       @click="clickRecording(item.patientDrugsTimes, drug.drugsTime)"
                       :style="{background : !isToday(drug.drugsTime) ? '#999' : stepActive === index ? '#40BD54' : '#91CB93'}"
                       style="color: white; background: rgb(145, 203, 147); border-radius: 7px; font-size: 12px; padding: 3px 10px;">
                    {{!isToday(drug.drugsTime) ? '打卡' : stepActive === index ? '打卡' : '补卡'}}
                  </div>
    ...
    <div style="width: 100%;  position: fixed; bottom: 15px; display: flex; justify-content: center">
      <van-button type="primary"
                  style="width: 90%;background: #67E0A7; border: none"
                  round @click="addMedication()">添加用药
      </van-button>
    </div>
      ...
      <div style="padding: 15px; max-height: 400px;">
        <div v-for="(item, index) in recordingList" :key="index" v-if="item.status === 2 && index < 7"
          ...
          <div @click="clickItem(item)"
            ...
            <div style="width: 90%">
              {{ (index + 1) + '. ' }}{{ item.medicineName }}({{ (item.drugsDose === -1 ? '外用' : item.drugsDose) + item.unit }})
            </div>
...
export default {
  ...
  mounted() {
    // 获取打卡记录
    if (this.$route.query && this.$route.query.drugsTimestamp) {
      this.getDrugsTime(this.$route.query.drugsTimestamp)
    }
    this.getData()
  ...
  methods: {
    ...
    clickDayDetail(item) {
      if (new Date() < new Date(item.date)) {
        this.$vux.toast.text('不能查看当前时间之后的日期', 'center')
      } else {
        this.stepActive = -1
        ...
        if (res.data && res.data.code === 0) {
          if (res.data.data) {
            ...
            for (let i = 0; i < this.list.length; i++) {
              if (this.getContrastTime(this.list[i].drugsTime) && this.stepActive === -1) {
                this.stepActive = i
              }
            }
          } else {
    ...
    clickRecording(item, drugsTime) {
      if (moment(drugsTime).format('YYYY-MM-DD HH') === moment().format('YYYY-MM-DD HH')) {
        let time = moment(drugsTime).format('YYYY-MM-DD HH:mm')
    ...
    addMedication() {
      if (this.$route.query && this.$route.query.imMessageId) {
        this.$router.push({ path: '/medicine/addmedicine', query: {imMessageId: this.$route.query.imMessageId}})
      }else {
