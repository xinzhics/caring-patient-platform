<template>
  <div style="position: relative">
            ...
            <div class="box" :style="{ borderRadius: openCurrentIndex === index ? '0px' : '10px' }">
              ...
              <div style="width: 20%; display: flex; justify-content: right">
                <van-switch :value="item.status" :active-color="'#66E0A7'" size="25px" :active-value="0"
                            :inactive-value="1" @input="currentSwitch(item)" active-color="#34C759"/>
              </div>
            </div>
            <template slot="right">
          ...
          <van-swipe-cell @open="openSwipe('history', index)" @close="closeSwipe('history', index)"
            ...
            <div class="box" style="background: #EAEAEA" :style="{ borderRadius: openHistoryIndex === index ? '0px' : '10px' }">
              <div style="width: 80%">
                ...
                <div style="display: flex; flex-flow: row wrap; align-items: flex-start;">
                  <div v-for="(itemDate, key) in item.patientDrugsTimeSettingList" :key="key"
                       style="margin-right: 10px; margin-top: 10px; font-size: 13px; color: #333; background: #B9B9B9; color: #FFF; border-radius: 5px;">
              ...
              <div style="width: 20%; display: flex; justify-content: right">
                <van-switch :value="item.status" :inactive-color="'#999999'" size="25px" :active-value="0"
                            :inactive-value="1" @input="historySwitch(item)"/>
              </div>
            </div>
            <template slot="right">
              <van-button square type="danger" text="删除" @click="deleteItem(item)"
    ...
    <div v-if="currentList.length === 0 && historyList.length === 0">
      <div class="nodata">
...
export default {
  ...
  data() {
    return {
      ...
      stopTips: false,
      Btnobj: {},
      noData: require('@/assets/my/medicineNoData.png'),
      openCurrentIndex: -1,
      openHistoryIndex: -1,
      stopReasonRemark: '',
  ...
  methods: {
    ...
    deleteItem(item) {
      Api.deletePatientDrugs(item.id)
        .then(res=> {
          this.getData()
        })
    },
    goItem(item) {
      this.$router.push({path: '/medicine/addmedicine', query: { medicineId : item.id}})
    },
    //打开
    ...
    getData() {
      ...
      Api.patientDrugsListAndHistory(params).then((res) => {
        if (res.data.code === 0) {
          this.currentList = res.data.data.medicines
    ...
    historySwitch(i) {
      i.status = 0
      Api.FixpatientDrugs(i).then((res) => {
        if (res.data.code === 0) {
          this.getData()
        }
      })
