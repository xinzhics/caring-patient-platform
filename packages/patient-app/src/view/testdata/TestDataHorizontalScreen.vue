<template>
  <section class="allContent" ref="view">
    ...
    <div style="background:#fff;display:flex;justify-content: space-between">
      ...
      <ul>
        <li v-for="(i,index) in timeData" :key="index" @click="chooseTime(i.monitorDayType,index)" v-if="index==0" class="firstli" :class="params.monitorDayType==i.monitorDayType?'chooseli ul-li firstli':'ul-li firstli'">{{i.name}}</li>
        <li v-if="index>0&&index<timeData.length-1" @click="chooseTime(i.monitorDayType,index)" v-for="(i,index) in timeData" :key="index" :class="params.monitorDayType==i.monitorDayType?'chooseli ul-li':'ul-li'">{{i.name}}</li>
        <li v-for="(i,index) in timeData" :key="index" @click="chooseTime(i.monitorDayType,index)" v-if="index===timeData.length-1" class="lastli" :class="params.monitorDayType==i.monitorDayType?'chooseli ul-li lastli':'ul-li lastli'">{{i.name}}</li>
      </ul>
    ...
    <div style="background:#fff">
      <div>
        <div class="ckqj" v-for="(item,index) in EchartsData.ydataList" :key="index">
          <div v-if="item.hasTrendReference!==false" class="ck">{{item.trendReference=='range'?'参考区间:':'参考值:'}}</div>
          <div style="color:#3f86ff;margin-left:10px" v-if="item.trendReference=='exact_value'&&item.trendReferenceExactValue!==null&&item.hasTrendReference!==false">{{item.trendReferenceExactValue}}</div>
          <div style="color:#3f86ff;margin-left:10px" v-if="item.trendReference=='range'&&item.hasTrendReference!==false">{{item.trendReferenceMin}}-{{item.trendReferenceMax}}</div>
        </div>
      </div>
    </div>
  </section>
...
export default {
  ...
  beforeMount() {
    ...
    if (this.$route.query.title) {
      ...
      console.log(this.$route.query.monitorDayType === 'OneDay')
      this.params.monitorDayType = this.$route.query.monitorDayType

      if (this.$route.query.monitorDayType === 'OneDay') {
  ...
  methods: {
    ...
    //折线图
    getEchartsData() {
      Api.getMonitorLineChart(this.$route.query.id, this.type).then(res => {
