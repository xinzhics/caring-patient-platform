<template>
  <section ref="mianscroll" class="allContent">
      <navBar @toHistoryPage="addData" :pageTitle="pageTitle ? pageTitle : '监测数据'"
              :rightIcon="require('@/assets/my/add.png')"
              backUrl="/monitor/index"
              :showRightIcon="!EchartsData.showTrend && list.length !== 0"/>
    ...
    <div v-show="(list.length > 0 || monitorQueryDate !== 'All') && EchartsData.showTrend==true"
      ...
      <div>
        <div class="ckqj" v-for="(item,index) in EchartsData.ydataList" :key="index"
          ...
          <div style="display: flex">
            <div v-if="item.showReferenceTargetValue===true" class="jzz">基线值: <span
                style="color: #5B97FF;">{{ item.referenceValue }}</span></div>
            <div v-if="item.showReferenceTargetValue===true" class="mbz">目标值: <span
    ...
    <div
      ...
      <select class="caring-select" style="width: 120px;" v-model="monitorQueryDate" @change="monitorQueryDateChange">
        <option v-for="(item, index) in monitorQueryDateList" :key="index" :value="item.code" >
          <div @click="() => monitorQueryDateClick(item.code)">
            {{ item.name }}
          </div>
        </option>
      </select>
    ...
    <div v-if="list.length === 0 && !loading">
      <div class="nodata">
        <img :src="require('@/assets/my/medicineNoData.png')" alt="" style="width: 80%">
      ...
      <van-pull-refresh v-model="refreshing" @refresh="onRefresh" :class="EchartsData.showTrend==false?'jiange':''"
        ...
        <van-list :finished="finished" :finished-text=finishedText() @load="onLoad">
          <div v-for="(i, k) in list" :key="k" @click="addData(i)">
            <van-swipe-cell v-if="i.scoreQuestionnaire === 1" style="width: auto; margin: 15px 10px; background-color: #fff; border-radius: 12px;">
              <div>
                <div style="display: flex; font-size: 14px">
                  <div style="color:#323233; padding: 17px 16px; width: calc(100% - 122px)">
                    <div>
                      <div slot="title" style="display: flex; align-items: center">
                        <div style="background: #08BDC0; width: 3px;border-radius: 4px; height: 16px; margin-right: 6px;"/>
                        <span style="color: #333333; font-size: 16px; font-weight: bold; margin-right: 10px; width: 70px;"> 监测时间 </span>
            ...
            <div v-else style="background: white; marginTop: 15px">
              <group>
                <cell>
                  <div slot="title" style=" margin-left: 5px;">
                    <span style="color: #333333; font-size: 16px; font-weight: 500; width: 70px;"> 监测时间 </span>
                    <span style="margin-left: 8px; width: 130px;"> {{
                      ...
                      <div v-if="getFormValue(item).type !== 'text'&&item.widgetType!=='Desc'" style="display: flex;">
                        <div style="display: flex">
                          ...
                          <flexbox style="margin-top: 5px;">
                            <flexbox-item v-for="(i, k) in getImage(item.values)" :key="k" v-if="k < 3">
      ...
      <div class="wrapper" @click.stop>
        <div class="block">
          <div class="title">
            ...
            <img style="position: absolute" @click="closeTime" src="../../images/close.png" alt="">
          </div>
...
import {ButtonTab, ButtonTabItem, Group} from 'vux'
...
export default {
  ...
  created() {
    ...
    this.current = 1
    //创建一个日期对象
    let time = JSON.parse(localStorage.getItem('myallInfo')).completeEnterGroupTime
    if (time) {
      //获取年份
  ...
  methods: {
    ...
    monitorQueryDateClick(code) {
      console.log('monitorQueryDateClick', code)
    },
    ...
    getchartdata() {
      ...
      Api.monitorLineChart(this.params, this.$route.query.id, patientId).then(res => {
        if (res.data.code === 0) {
            ...
            if (
              ...
              this.EchartsData.ymonitorEvent[0].ydata.forEach((item, index) => {
                if (item !== null) {
                  ...
                  ;(this.Ydata[index].symbolSize = [10, 20]), (this.Ydata[index].symbolOffset = [2, -8])
                }
              })
    ...
    iskuanian() {
      if (this.EchartsData.xdataList && this.EchartsData.xdataList.length > 0) {
        ...
        if (b - a > 0) {
          this.Xdata = []
          this.options.xAxis.axisLabel.rotate = 45
          this.EchartsData.xdataList.forEach(item => {
            let a = ''
            a = item.slice(0, 10)
            this.Xdata.push(a)
    ...
    //折线图
    getEchartsData() {
      Api.getMonitorLineChart(this.$route.query.id, this.params, localStorage.getItem('userId')).then(res => {
        if (res.data && res.data.code === 0) {
          this.showTrend = res.data.data.showTrend
          if (res.data.data.xdataList) {
            res.data.data.xdataList.forEach(item => {
              this.xList.push(this.getEchartsTime(item))
            })
          } else {
            this.xList = []
          }
          if (res.data.data.ydataList) {
            res.data.data.ydataList.forEach(item => {
              if (
    ...
    getData() {
      ...
      this.loading = true
      Api.getMonitorList(params).then(res => {
        if (res.data && res.data.code === 0) {
