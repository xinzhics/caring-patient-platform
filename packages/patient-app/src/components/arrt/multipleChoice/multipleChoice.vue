<template>
  ...
  <div>
    <div class="main">
      ...
      <van-cell v-if="noValues" :class="field.modifyTags?'blue-border':''" title="请选择" :ref="field.id" is-link @click="showPopup"></van-cell>
      <van-cell is-link :ref="field.id" @click="showPopup" v-if="!noValues" :class="field.modifyTags?'blue-border':''">
        <!-- 一般选项 -->
      ...
      <van-popup v-model="show" closeable close-icon-position="top-right" position="bottom">
          ...
          <div class="choose" v-for='(info,index) in field.options' :key="index">
            ...
            <div class="input1" v-if="info.select">
              ...
              <van-field v-if="field.itemAfterHasEnter === 1 && info.attrValue !== '其他'"
                         class="cell2"
                         :error="info.error"
                         rows="1"
                         autosize type="textarea" v-model="info.valuesRemark"
              ...
              <!-- 其他输入框 -->
              <van-field v-if="field.hasOtherOption === 1 && info.attrValue === '其他'"
                         class="cell2"
                         rows="1"
                         :error="field.error"
                         @input="(val) => fieldInputOther(val, info)"
                         autosize type="textarea"
                         v-model="field.otherValue" :placeholder="field.otherEnterRequired === 1? '请输入(必填)' : '请输入(选填)'"
                         show-word-limit
          ...
          <div class="submit">
            <van-button round type="info"
                        :disabled="checkCheckBoxDisableSubmit()"
                        @click="updata" >确定</van-button>
          </div>
      </van-popup>
    </div>
    <div v-if="field.values && field.values.length > 0" v-for='(value,k) in field.values' :key="field.id + 'question' +  k">
