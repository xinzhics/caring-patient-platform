    ...
    <van-col span="24" v-for="(field, index) in currentFields" :key="index" class="attr-col">
      ...
      <splitLine v-if="field.widgetType === Constants.CustomFormWidgetType.SplitLine" :field="field"></splitLine>
       <van-col v-if="field.widgetType === Constants.CustomFormWidgetType.SingleLineText
        ...
        <van-cell title-class="van-cell-width50" label-class="van-cell-width50" class="mycell" :value="field.values && field.values[0]&& field.values[0].attrValue? field.values[0].attrValue : '-'">
          <template slot="title">
            <div>{{ field.label }}</div>
          </template>
...
<!--      <van-col v-if="field.widgetType === Constants.CustomFormWidgetType.SingleLineText" span="24">-->
<!--        <singleLine :field="field" :disabled="true" />-->
<!--      </van-col>-->
      <van-col v-if="field.widgetType === Constants.CustomFormWidgetType.MultiLineText" span="24">
        <van-cell title-class="van-cell-width50" label-class="van-cell-width50" :value="field.values && field.values[0]&&field.values[0].attrValue ? field.values[0].attrValue : '-'">
          <!-- <template #title> -->
      ...
      <van-col v-if="field.widgetType === Constants.CustomFormWidgetType.Radio
        ...
        <van-cell title-class="van-cell-width50" label-class="van-cell-width50">
      ...
      <!--头像-->
      <van-col v-if="field.exactType === Constants.FieldExactType.Avatar" span="24" style="text-align: center; background-color: #fff; height: 120px; padding-top: 30px">
        <template v-for="(v,k) in field.values">
          <img v-if="v.attrValue" :src="v.attrValue" :key="k" alt class="med-img" @click="showImage(v.attrValue)" />
          <img v-if="!v.attrValue" :src="headerImgT" :key="k" alt class="med-img" />
        </template>
      ...
      <van-col v-if="field.widgetType === Constants.CustomFormWidgetType.Address" span="24">
        <van-cell title-class="van-cell-width50" label-class="van-cell-width50" :title="field.label">
          <!-- <template #title> -->
          <div slot="title">{{ field.label }}</div>
          <!-- </template> -->
  ...
  methods: {
    ...
    getDropdownSelectResult(values) {
      if (values && values.length > 0) {
        const values1 = values[0]
        if (values1 && values1.children) {
          if (values1.children.length > 0) {
            for (let i = 0; i < values1.children.length; i++) {
              if (values1.children[i].values && values1.children[i].values.length > 0) {
                return values1.children[i].values[0].valueText
              }
            }
          }
        }
        return values1.valueText
      } else {
        return '-'
      }
    },
    ...
    getFieldsResult(values, otherValue) {
      ...
      if (values && values.length > 0) {
        ...
        for (let i = 0; i < values.length; i++) {
          ...
          } else if (values[i].valueText && values[i].valueText.length > 0) {
            value = values[i].valueText
          } else if (values[i].attrValue && values[i].attrValue.length > 0) {
            value = values[i].attrValue
          }
