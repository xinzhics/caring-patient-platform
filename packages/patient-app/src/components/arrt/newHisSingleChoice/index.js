...
export default {
  ...
  mounted () {
    console.log('新题单选',this.field)
    if (this.field&&this.field.values[0]&&this.field.values[0].valueText&&this.isHealth === '1') {
      this.noClick = true
    }
    if (this.field.values[0].desc) {
      this.$refs[this.field.id].children[1].style = "display:block";
    }
    ...
    if (this.field) {
      if (this.field.values && this.field.values.length > 0) {
        ...
        console.log(this.$refs[this.field.id].firstChild.firstChild);
