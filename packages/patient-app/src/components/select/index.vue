<template>
  <div>
    <van-field v-model="showText" :border="false" :placeholder="placeholder||'请选择'" readonly :error-message-align="errorMessageAlign" :error-message='errorMessage' @click="show = !show" />
    <van-popup v-model="show" position="bottom">
      <van-picker type="address" :columns="columns" show-toolbar :title="$attrs.label" value-key="name" @confirm="onConfirm" @cancel="show = !show" />
    </van-popup>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      selText: '',
      resultData: this.selVal && typeof this.selVal == 'object' ? JSON.parse(JSON.stringify(this.selVal)) : this.selVal
    }
  },
  methods: {
    onConfirm(value, ind) {
      ...
      if (this.surecb) {
        console.log('surecb', this.resultData, this.sourceData, this.selInd)
        this.nextTick(() => {
    ...
    setInitResult() {
      ...
      if (!this.type && this.columns && this.columns.length > 0) {
        if (!this.resultData) {
          ...
          this.resultData =
            this.value && typeof this.value == 'object' ? JSON.parse(JSON.stringify(this.value)) : this.value
        }
        ...
        if (this.resultData) {
          ...
          if (this.selInd >= 0) {
            this.$set(this, 'resultData', JSON.parse(JSON.stringify(this.columns[this.selInd])))
          }
          this.$set(this, 'showText', this.resultData.text || this.resultData.valueText || this.resultData)
        }
      }
    },
    initSourceData() {
      if (this.selVal && typeof this.selVal == 'object') this.sourceData = this.selVal
    }
  },
  mounted() {
    this.setInitResult()
