<template>
  <div class="lano-uplodaer-container" :class='className' style="background: #FFFFFF">
    <!--上传如果是头像 使用头像布局-->
    <!--上传图片-->
    <div class="lano-uploader-label" :class="{'required':required}">
      <van-row>
        <van-col span="7">
          <span style="font-size: 15px; color: #333333" v-text="label"></span>
        </van-col>
        <van-col span="17">
          <div class="lano-upload-wrap" style="float: right">
            <div class="lano-upload-wrap" style="float: right">
              <div class="lano-upload-preview" v-for="(file,ind) in fileList" :key="ind">
                <van-image :src="file[keyName]" :alt="label" @click="showImage()">
              ...
              :after-read="onUpload"
              :multiple="false" v-if="fileList.length<(max?Number(max):1)" :before-read="beforeReadFun" :max-count="max?Number(max):1" />
            </div>
          </div>
        </van-col>
      </van-row>
      <p class="error-msg" v-text="'请选择'+label||'文件'" v-if="errorMessage && required"></p>
    </div>
    <x-dialog v-model="showDialog" class="dialog-demo">
      <p style="margin-top:20px">是否确定删除此图片！</p>
      <div style="display: flex;justify-content: space-between;margin:20px 0px">
        <x-button mini type="warn" action-type="reset" style="height:30px;padding:0px 30px" @click.native="detelBtn">
          确定
        </x-button>
        <x-button mini type="primary" action-type="button" style="height:30px;margin-top:0px;padding:0px 30px" @click.native="cancelBtn">取消
        </x-button>
      </div>
    </x-dialog>
    <loading :show="isLoading" @click="isLoading = true" />
  </div>
</template>
