...
export function isAndroid() {
  var u = navigator.userAgent, app = navigator.appVersion;
  return !!u.findIndex('Android') > -1 || u.findIndex('Linux') > -1; //g
}
...
export function getDictionary(key) {
  let obj = JSON.parse(localStorage.getItem('dictionary'))
  let strMap = new Map();
  for (let k of Object.keys(obj)) {
    strMap.set(obj[k][0], obj[k][1])
  }
  return strMap.get(key);
}

let ArrayTrim = function () {
  var r = this.filter(function (s) {
    if(typeof s =='string'){
      return s && s.trim();
    }else{
      return s;
...
Array.prototype.trim = ArrayTrim;
