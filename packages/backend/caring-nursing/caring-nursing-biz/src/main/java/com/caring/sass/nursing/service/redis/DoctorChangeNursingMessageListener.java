...
public class DoctorChangeNursingMessageListener implements MessageListener {
    ...
    public void onMessage(Message message, byte[] pattern) {
        ...
        if (SaasRedisBusinessKey.DOCTOR_CHANGE_NURSING.equals(channel)) {
            ...
            String handleKey = SaasRedisBusinessKey.getDoctorChangeNursingHandleKey("nursing",
                    messageDto.getDoctorId() , messageDto.getTargetNursingId());
            handleMessage(messageDto, handleKey);
        }

    }

    protected void handleMessage(RedisMessageDoctorChangeNursing message, String handleKey) {
        RedisMessageHandle messageHandle = getRedisMessageHandle();
        if (messageHandle == null) {
            log.info("RedisMessageHandle not find");
        } else {
            messageHandle.handleDoctorChangeNursingMessage(message, handleKey);
        }
    }
