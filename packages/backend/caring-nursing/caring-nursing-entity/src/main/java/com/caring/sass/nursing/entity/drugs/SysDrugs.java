package com.caring.sass.nursing.entity.drugs;

import cn.afterturn.easypoi.excel.annotation.Excel;
import com.baomidou.mybatisplus.annotation.TableField;
import com.baomidou.mybatisplus.annotation.TableName;
import com.caring.sass.base.entity.Entity;
import io.swagger.annotations.ApiModel;
import io.swagger.annotations.ApiModelProperty;
import lombok.*;
import lombok.experimental.Accessors;
import org.hibernate.validator.constraints.Length;

import java.time.LocalDateTime;
import java.util.List;

import static com.baomidou.mybatisplus.annotation.SqlCondition.EQUAL;
import static com.baomidou.mybatisplus.annotation.SqlCondition.LIKE;

/**
 * <p>
 * 实体类
 * 药品
 * </p>
 *
 * @author leizhi
 * @since 2020-09-15
 */
@Data
@NoArgsConstructor
@ToString(callSuper = true)
@EqualsAndHashCode(callSuper = true)
@Accessors(chain = true)
@Builder
@TableName("t_sys_drugs")
@ApiModel(value = "SysDrugs", description = "药品")
@AllArgsConstructor
public class SysDrugs extends Entity<Long> {

    private static final long serialVersionUID = 1L;

    /**
     * 名字
     */
    @ApiModelProperty(value = "名字")
    @Length(max = 60, message = "名字长度不能超过60")
    @TableField(value = "name", condition = LIKE)
    @Excel(name = "名字")
    private String name;

    /**
     * 通用名
     */
    @ApiModelProperty(value = "通用名")
    @Length(max = 60, message = "通用名长度不能超过60")
    @TableField(value = "generic_name", condition = LIKE)
    @Excel(name = "通用名")
    private String genericName;

    /**
     * 图标
     */
    @ApiModelProperty(value = "图标")
    @Length(max = 500, message = "图标长度不能超过500")
    @TableField(value = "icon", condition = LIKE)
    @Excel(name = "图标")
    private String icon;

    /**
     * 国药准字
     */
    @ApiModelProperty(value = "国药准字")
    @Length(max = 32, message = "国药准字长度不能超过32")
    @TableField(value = "gyzz", condition = LIKE)
    @Excel(name = "国药准字")
    private String gyzz;

    /**
     * 厂家
     */
    @ApiModelProperty(value = "厂家")
    @Length(max = 100, message = "厂家长度不能超过100")
    @TableField(value = "manufactor", condition = LIKE)
    @Excel(name = "厂家")
    private String manufactor;

    /**
     * 规格
     */
    @ApiModelProperty(value = "规格")
    @Length(max = 400, message = "规格长度不能超过400")
    @TableField(value = "spec", condition = LIKE)
    @Excel(name = "规格")
    private String spec;

    /**
     * 用法
     */
    @ApiModelProperty(value = "用法")
    @Length(max = 2500, message = "用法长度不能超过2500")
    @TableField(value = "usage_", condition = LIKE)
    @Excel(name = "用法")
    @Deprecated
    private String usage;

    /**
     * 用量
     */
    @ApiModelProperty(value = "用法用量")
    @Length(max = 65535, message = "用量长度不能超过65535")
    @TableField("dosage")
    @Excel(name = "用法用量")
    private String dosage;

    /**
     * 说明书
     */
    @ApiModelProperty(value = "说明书")
    @Length(max = 65535, message = "说明书长度不能超过65535")
    @TableField("instructions")
    @Excel(name = "说明书")
    private String instructions;

    /**
     * 适用症
     */
    @ApiModelProperty(value = "适用症")
    @Length(max = 65535, message = "适用症长度不能超过65535")
    @TableField("applicable_disease")
    @Excel(name = "适用症")
    private String applicableDisease;


    /**
     * 禁忌症
     */
    @ApiModelProperty(value = "禁忌症")
    @Length(max = 65535, message = "禁忌症长度不能超过65535")
    @TableField("taboo")
    @Excel(name = "禁忌症")
    private String taboo;

    /**
     * 不良反应
     */
    @ApiModelProperty(value = "不良反应")
    @Length(max = 65535, message = "不良反应长度不能超过65535")
    @TableField("side_effects")
    @Excel(name = "不良反应")
    private String sideEffects;

    /**
     * 药物相互作用
     */
    @ApiModelProperty(value = "药物相互作用")
    @Length(max = 65535, message = "药物相互作用长度不能超过65535")
    @TableField("interaction")
    @Excel(name = "药物相互作用")
    private String interaction;

    /**
     * 删除标记(0：未删除)
     */
    @ApiModelProperty(value = "删除标记(0：未删除)")
    @TableField("del_flag")
    @Excel(name = "删除标记(0：未删除)")
    private Integer delFlag;

    /**
     * 每日用药次数
     */
    @ApiModelProperty(value = "每日用药次数")
    @TableField("number")
    @Excel(name = "每日用药次数")
    private Integer number;

    /**
     * 每次剂量
     */
    @ApiModelProperty(value = "每次剂量")
    @TableField("dose")
    @Excel(name = "每次剂量")
    private Float dose;

    /**
     * 用药时间
     */
    @ApiModelProperty(value = "用药时间")
    @Length(max = 50, message = "用药时间长度不能超过50")
    @TableField(value = "time", condition = LIKE)
    @Excel(name = "用药时间")
    private String time;

    /**
     * 用药周期(0:长期   N :N天)
     */
    @ApiModelProperty(value = "用药周期(0:长期   N :N天)")
    @TableField("cycle")
    @Excel(name = "用药周期(0:长期   N :N天)")
    private Integer cycle;

    /**
     * 单位（粒、瓶、毫升等）
     */
    @ApiModelProperty(value = "单位（粒、瓶、毫升等）")
    @Length(max = 20, message = "单位（粒、瓶、毫升等）长度不能超过20")
    @TableField(value = "unit", condition = LIKE)
    @Excel(name = "单位（粒、瓶、毫升等）")
    private String unit;

    /**
     * 条码
     */
    @ApiModelProperty(value = "条码")
    @Length(max = 50, message = "条码长度不能超过50")
    @TableField(value = "code", condition = LIKE)
    @Excel(name = "条码")
    private String code;

    /**
     * 汉语拼音首字母
     */
    @ApiModelProperty(value = "汉语拼音首字母")
    @Length(max = 200, message = "汉语拼音首字母长度不能超过200")
    @TableField(value = "pyszm", condition = EQUAL)
    @Excel(name = "汉语拼音首字母")
    private String pyszm;

    /**
     * 药品简称或者说俗名
     */
    @ApiModelProperty(value = "药品简称或者说俗名")
    @Length(max = 200, message = "药品简称或者说俗名长度不能超过200")
    @TableField(value = "commondity_name", condition = LIKE)
    @Excel(name = "药品简称或者说俗名")
    private String commondityName;

    /**
     * 组成成分
     */
    @ApiModelProperty(value = "组成成分")
    @Length(max = 200, message = "组成成分长度不能超过200")
    @TableField(value = "characters", condition = LIKE)
    @Excel(name = "组成成分")
    private String characters;

    /**
     * 英文名称
     */
    @ApiModelProperty(value = "英文名称")
    @Length(max = 200, message = "英文名称长度不能超过200")
    @TableField(value = "english_name", condition = LIKE)
    @Excel(name = "英文名称")
    private String englishName;

    /**
     * 执行标准
     */
    @ApiModelProperty(value = "执行标准")
    @Length(max = 200, message = "执行标准长度不能超过200")
    @TableField(value = "c_executive_standard", condition = LIKE)
    @Excel(name = "执行标准")
    private String executiveStandard;

    /**
     * 是否MI用药
     */
    @ApiModelProperty(value = "是否MI用药")
    @TableField("is_mi")
    @Excel(name = "是否MI用药", replace = {"是_true", "否_false", "_null"})
    private Boolean isMi;

    /**
     * 是否OTC用药
     */
    @ApiModelProperty(value = "是否OTC用药")
    @TableField("is_otc")
    @Excel(name = "是否OTC用药", replace = {"是_true", "否_false", "_null"})
    private Boolean isOtc;

    /**
     * 包装
     */
    @ApiModelProperty(value = "包装")
    @Length(max = 200, message = "包装长度不能超过200")
    @TableField(value = "pack", condition = LIKE)
    @Excel(name = "包装")
    private String pack;

    /**
     * 储藏说明
     */
    @ApiModelProperty(value = "储藏说明")
    @Length(max = 200, message = "储藏说明长度不能超过200")
    @TableField(value = "storage", condition = LIKE)
    @Excel(name = "储藏说明")
    private String storage;

    /**
     * 分类ID
     */
    @ApiModelProperty(value = "分类ID (已弃用)")
    @Length(max = 200, message = "分类ID长度不能超过200")
    @TableField(value = "category_id", condition = EQUAL)
    @Excel(name = "分类ID")
    @Deprecated
    private Long categoryId;

    /**
     * 药品有效期
     */
    @ApiModelProperty(value = "药品有效期")
    @Length(max = 200, message = "药品有效期长度不能超过200")
    @TableField(value = "valid_time", condition = LIKE)
    @Excel(name = "药品有效期")
    private String validTime;

    /**
     * 是否置顶 1 置顶  -1 不置顶
     */
    @ApiModelProperty(value = "是否置顶 1 置顶  -1 不置顶")
    @TableField("sticky")
    @Excel(name = "是否置顶 1 置顶  -1 不置顶")
    private Integer sticky;

    @ApiModelProperty(value = "置顶时间")
    @TableField(value = "sticky_time")
    private LocalDateTime stickyTime;

    /**
     * 冗余字段 类别名称
     */
    @ApiModelProperty(value = "冗余字段 类别名称 (已弃用)")
    @TableField(exist = false)
    private String categoryName;

    @ApiModelProperty(value = "剂型")
    @Length(max = 200, message = "剂型")
    @TableField(value = "dosage_form")
    private String dosageForm;

    @ApiModelProperty(value = "爬取的链接地址")
    @TableField(value = "link")
    private String link;

    @ApiModelProperty(value = "爬取药品")
    @TableField(value = "reptile")
    private Integer reptile;

    @ApiModelProperty(value = "爬取药品详情状态")
    @TableField(value = "reptile_ok")
    private Integer reptileOk;

    @ApiModelProperty(value = "冗余字段药品类别")
    @TableField(exist = false)
    private List<SysDrugsCategoryLink> sysDrugsCategoryLinkList;

    @ApiModelProperty(value = "冗余字段详细图片列表")
    @TableField(exist = false)
    private List<SysDrugsImage> sysDrugsImages;

    @ApiModelProperty(value = "药量计数")
    @TableField(value = "dosage_count")
    private Integer dosageCount;

}
