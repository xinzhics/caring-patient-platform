...
public class RoleController extends SuperCacheController<RoleService, Long, Role, RolePageDTO, RoleSaveDTO, RoleUpdateDTO> {
    ...
    @GetMapping(value = "/findUserByRoleId/{userId}")
    public R< List<Role>> findUserByRoleId(@PathVariable("userId") Long userId) {
        List<Role> list = baseService.findRoleByUserId(userId);
