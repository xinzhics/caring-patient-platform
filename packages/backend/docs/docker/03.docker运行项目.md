...
```
    ...
    -e MYSQL_ROOT_PASSWORD="${MYSQL_ROOT_PASSWORD:-root}" \                     # 数据库访问密码
```
- 启动
```
...
- 修改 初始密码账号、路径
```
-e RABBITMQ_DEFAULT_USER=caring -e RABBITMQ_DEFAULT_PASS=caring \       # 账号密码    
...
```
- 将 caring.conf 文件中的所有 localhost 替换成宿主机的内网ip。 如： 192.168.1.1
- 启动
```
...
cd caring-backend/caring-authority/caring-authority-server
docker build --build-arg JAR_FILE=target/caring-authority-server.jar -t caring-authority-server:v1 .

cd caring-backend/caring-msgs/caring-msgs-server
docker build -t caring-msgs-server:v1 .

cd caring-backend/caring-file/caring-file-server
docker build -t caring-file-server:v1 .

cd caring-backend/caring-gateway/caring-zuul-server
docker build -t caring-zuul-server:v1 .

cd caring-backend/caring-jobs/caring-jobs-server
docker build -t caring-jobs-server:v1 .
```

## 7，使用docker启动项目
```
1, 确保： nacos mysql redis 全部启动， 并且nacos 里面的redis mysql 等ip改成自己的ip （切记，使用docker时，所有的IP都不能为127.0.0.1）
2, 在nacos 里面新建  caring-authority-server-docker.yml ,并将所有caring-authority-server-dev.yml 里面的配置复制过去，修改docker环境和dev环境的差异部分部分。

docker run -idt  --name caring-authority-server --restart=always \
        ...
        -p 8764:8764 \
        -v /data/:/data \
        caring-authority-server:v1 

docker run -idt  --name caring-zuul-server --restart=always \
        -e NACOS_IP=192.168.1.34 \
        -e NACOS_PORT=8848 \
        -e LOCAL_IP=192.168.1.34 \
        -e NACOS_ID=b16f7baf-56e7-4f4e-a26c-425ee0668016 \
        -p 8760:8760 \
        -v /data/:/data \
        caring-zuul-server:v1 

docker run -idt  --name caring-msgs-server --restart=always \
        -e NACOS_IP=192.168.1.34 \
        -e NACOS_PORT=8848 \
        -e LOCAL_IP=192.168.1.34 \
        -e NACOS_ID=b16f7baf-56e7-4f4e-a26c-425ee0668016 \
        -p 8768:8768 \
        -v /data/:/data \
        caring-msgs-server:v1 

docker run -idt  --name caring-file-server --restart=always \
        -e NACOS_IP=192.168.1.34 \
        -e NACOS_PORT=8848 \
        -e LOCAL_IP=192.168.1.34 \
        -e NACOS_ID=b16f7baf-56e7-4f4e-a26c-425ee0668016 \
        -p 8765:8765 \
        -v /data/:/data \
        caring-file-server:v1 

docker run -idt  --name caring-jobs-server --restart=always \
        -e MYSQL_IP=192.168.1.34 \
        -p 8767:8767 \
        -v /data/:/data \
        caring-jobs-server:v1 
...
docker run -idt  --name caring-nacos --restart=always \
