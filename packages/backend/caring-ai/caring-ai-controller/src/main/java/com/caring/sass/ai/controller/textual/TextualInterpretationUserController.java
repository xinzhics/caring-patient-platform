...
@Validated
@RestController
@RequestMapping("/textualInterpretationUser")
@Api(value = "TextualInterpretationUser", tags = "文献解读用户表")
public class TextualInterpretationUserController {
    ...
    @Autowired
    TextualInterpretationUserService baseService;
    ...
    public R<TextualUserPayConfig> queryCost() {
        return R.success(textualUserPayConfig);
    }
    ...
    public R<ArticleUserInfoUpdateDTO> refreshToken(@RequestBody ArticleUserInfoUpdateDTO articleUserInfoUpdateDTO) {
        ...
        TextualInterpretationUser user = baseService.getById(userId);
        if (user == null) {
    ...
    public R<Boolean> updatePassword(@RequestBody @Validated UserUpdatePasswordDTO updatePassword) {
        ...
        TextualInterpretationUser user = new TextualInterpretationUser();
        user.setId(id);
    ...
    public R<Boolean> resetPassword(@RequestBody @Validated UserResetPasswordDTO userResetPassword) {
        ...
        TextualInterpretationUser user = null;
        try {
            user = baseService.getOne(Wraps.<TextualInterpretationUser>lbQ()
                    .eq(TextualInterpretationUser::getUserMobile, EncryptionUtil.encrypt(mobile))
                    .last(" limit 0,1 "));
        } catch (Exception e) {
    ...
    public R<Boolean> checkMobileExist(@RequestParam String userAccount) {
        try {
            TextualInterpretationUser user = baseService.getOne(Wraps.<TextualInterpretationUser>lbQ()
                    .eq(TextualInterpretationUser::getUserMobile, EncryptionUtil.encrypt(userAccount))
                    .last(" limit 0,1 "));
    ...
    public R<JSONObject> register(@RequestBody @Validated ArticleUserSaveDTO articleUserSaveDTO) throws Exception {
        ...
        if (StrUtil.isNotBlank(password)) {
            ...
            TextualInterpretationUser user = baseService.getOne(Wraps.<TextualInterpretationUser>lbQ()
                    .eq(TextualInterpretationUser::getUserMobile, EncryptionUtil.encrypt(userMobile))
                    .last(" limit 0,1 "));
            if (Objects.nonNull(user)) {
                return R.fail("账号已存在");
            }
            user = new TextualInterpretationUser()
    ...
    public boolean sendSms(@RequestBody SmsDto smsDto) {
        ...
        if (password != null && password) {
            try {
                int count = baseService.count(Wraps.<TextualInterpretationUser>lbQ()
                        .eq(TextualInterpretationUser::getUserMobile, EncryptionUtil.encrypt(phone)));
                if (count == 0) {
    ...
    public R<TextualInterpretationUser> getUserInfo(Long userId) {

        TextualInterpretationUser articleUser = baseService.getById(userId);
