 ...
 */
@Slf4j
public class PodcastDifyBotSSEEventSourceListener extends EventSourceListener {
    ...
    public PodcastDifyBotSSEEventSourceListener(PodcastMapper podcastMapper, RedisTemplate<String, String> redisTemplate, String uid) {
        this.uid = uid;
        ...
        try {
            ...
            if ("error".equals(event)) {
                ...
                if ("invalid_param".equals(code)) {
                    sendMessage("您输入的内容包含敏感信息，本次创作将失败，稍后会为您退费，您可以选择开启一个新创作！");
                } else {
                    sendMessage("AI 生成内容时遇到了问题，本次创作将失败，稍后会为您退费，您可以选择开启一个新创作！");
                }
            }
        } catch (Exception e) {
            log.error("sse信息推送失败！");
    ...
    public void sendMessage(String message) {
        ...
        redisTemplate.opsForList().leftPush("podcast:" + uid, message);
    }
