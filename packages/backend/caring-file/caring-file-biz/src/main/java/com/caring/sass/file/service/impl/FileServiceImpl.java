...
import com.caring.sass.file.biz.FileBiz;
import com.caring.sass.file.dao.FileMapper;
import com.caring.sass.file.domain.FileAttrDO;
import com.caring.sass.file.domain.FileDO;
...
import static com.caring.sass.utils.StrPool.DEF_ROOT_PATH;
...
public class FileServiceImpl extends SuperServiceImpl<FileMapper, File> implements FileService {
     ...
     * @return
     */
    public String getFileMd5(MultipartFile simpleFile) {
        DataType dataType = FileDataTypeUtil.getDataType(simpleFile.getContentType());
        if (DataType.IMAGE.eq(dataType)) {
            try {
                byte[] bytes = simpleFile.getBytes();
    ...
    public File upload(MultipartFile simpleFile, Long folderId) {
        ...
        // 图片文件进行md5
        String fileMd5 = getFileMd5(simpleFile);
        if (StrUtil.isNotEmpty(fileMd5)) {
            File one = super.getOne(Wraps.<File>lbQ().eq(File::getFileMd5, fileMd5).last(" limit 0,1 "));
        ...
        if (Objects.equals(BaseContextHandler.getTenant(), BizConstant.SUPER_TENANT)) {
            file.setOwnerType(OwnerTypeEnum.SYS);
        }
        super.save(file);
        return file;
    }
    ...
    public File saveFile(FileSaveDto fileSaveDto) {
        ...
        Long folderId = fileSaveDto.getFolderId();
        FileAttrDO fileAttrDO = this.getFileAttrDo(folderId);
        String treePath = fileAttrDO.getTreePath();
        String folderName = fileAttrDO.getFolderName();
        Integer grade = fileAttrDO.getGrade();
        File file = File.builder()
                .isDelete(false)
                .contextType(fileSaveDto.getContextType())
                .dataType(fileSaveDto.getDataType())
                .size(fileSaveDto.getSize())
                .ext(fileSaveDto.getExt())
                .url(fileSaveDto.getUrl())
                .folderId(folderId)
                .folderName(folderName)
                .grade(grade)
                .treePath(treePath)
                .submittedFileName(fileSaveDto.getSubmittedFileName())
                .build();
        if (Objects.equals(BaseContextHandler.getTenant(), BizConstant.SUPER_TENANT)) {
            file.setOwnerType(OwnerTypeEnum.SYS);
        }
    ...
    @Override
    public File uploadAppFile(MultipartFile simpleFile, Long folderId, String obsFileName) {
        FileAttrDO fileAttrDO = this.getFileAttrDo(folderId);
        String treePath = fileAttrDO.getTreePath();
        String folderName = fileAttrDO.getFolderName();
        Integer grade = fileAttrDO.getGrade();
        ...
        File file = fileStrategy.uploadAppFile(simpleFile, obsFileName);
        file.setFolderId(folderId);
        file.setFolderName(folderName);
        file.setGrade(grade);
        file.setTreePath(treePath);
    ...
    @Override
    public List<File> listNoCodeByIds(List<Long> fileIds) {
        return baseMapper.listNoCodeByIds(fileIds);
    }
