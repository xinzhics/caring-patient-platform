<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button @click="add" class="filter-item" plain type="danger" v-has-permission="['channel:add']">
        {{ $t("table.add") }}
      </el-button>
      <el-dropdown class="filter-item" trigger="click" v-has-any-permission="['channel:delete', 'channel:export',
        'channel:import']">
        <el-button>
          {{ $t("table.more") }}<i class="el-icon-arrow-down el-icon--right" />
        </el-button>
        <el-dropdown-menu slot="dropdown">
          <el-dropdown-item @click.native="batchDelete" v-has-permission="['channel:delete']">
            {{ $t("table.delete") }}
          </el-dropdown-item>
          <el-dropdown-item @click.native="exportExcel" v-has-permission="['channel:export']">
            {{ $t("table.export") }}
          </el-dropdown-item>
          <el-dropdown-item @click.native="exportExcelPreview" v-has-permission="['channel:export']">
            {{ $t("table.exportPreview") }}
          </el-dropdown-item>
          <el-dropdown-item @click.native="importExcel" v-has-permission="['channel:import']">
            {{ $t("table.import") }}
          </el-dropdown-item>
        </el-dropdown-menu>
      </el-dropdown>
    ...
    <channel-edit :dialog-visible="dialog.isVisible" :type="dialog.type"
            @close="editClose" @success="editSuccess" :doctorOwner="doctorOwner" ref="edit"/>
...
export default {
  ...
  data() {
    return {
      ...
      fileImport: {
        isVisible: false,
        type: "import",
        action: `${process.env.VUE_APP_BASE_API}/cms/channel/import`
      },
      tableKey: 0,
  ...
  methods: {
    ...
    exportExcelPreview() {
      ...
      channelApi.preview(this.queryParams).then(response => {
    ...
    exportExcel() {
      ...
      this.queryParams.map.fileName = '导出数据';
      channelApi.export(this.queryParams).then(response => {
    ...
    delete(ids) {
      channelApi.delete({ ids: ids }).then(response => {
        const res = response.data;
        if (res.isSuccess) {
          this.$message({
    ...
    add() {
      ...
      this.$refs.edit.setChannel({ enums: this.enums, dicts: this.dicts});
    },
    copy(row) {
      this.$refs.edit.setChannel({row, enums: this.enums, dicts: this.dicts});
    ...
    fetch(params = {}) {
      ...
      channelApi.page(this.queryParams).then(response => {
        const res = response.data;
        if (res.isSuccess) {
