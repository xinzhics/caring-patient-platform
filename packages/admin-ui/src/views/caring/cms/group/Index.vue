<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button @click="add" class="filter-item" plain type="danger" v-has-permission="['group:add']">
        {{ $t("table.add") }}
      </el-button>
      <el-dropdown class="filter-item" trigger="click" v-has-any-permission="['group:delete', 'group:export',
        'group:import']">
        <el-button>
          {{ $t("table.more") }}<i class="el-icon-arrow-down el-icon--right" />
        </el-button>
        <el-dropdown-menu slot="dropdown">
          <el-dropdown-item @click.native="batchDelete" v-has-permission="['group:delete']">
            {{ $t("table.delete") }}
          </el-dropdown-item>
          <el-dropdown-item @click.native="exportExcel" v-has-permission="['group:export']">
            {{ $t("table.export") }}
          </el-dropdown-item>
          <el-dropdown-item @click.native="exportExcelPreview" v-has-permission="['group:export']">
            {{ $t("table.exportPreview") }}
          </el-dropdown-item>
          <el-dropdown-item @click.native="importExcel" v-has-permission="['group:import']">
            {{ $t("table.import") }}
          </el-dropdown-item>
        </el-dropdown-menu>
      </el-dropdown>
    ...
    <el-table :data="tableData.records" :key="tableKey" @cell-click="cellClick"
      ...
      <el-table-column :label="$t('table.group.nurseName')" :show-overflow-tooltip="true" align="center" prop="nurseName"
                        width="">
        <template slot-scope="scope">
      ...
      <el-table-column :label="$t('table.group.name')" :show-overflow-tooltip="true" align="center" prop="name"
                        width="">
        <template slot-scope="scope">
      ...
      <el-table-column
        ...
        <template slot-scope="{ row }">
          <i @click="copy(row)" class="el-icon-copy-document table-operation" :title="$t('common.delete')"
             style="color: #2db7f5;" v-hasPermission="['group:add']"/>
          <i @click="edit(row)" class="el-icon-edit table-operation" :title="$t('common.delete')"
             style="color: #2db7f5;" v-hasPermission="['group:update']"/>
          <i @click="singleDelete(row)" class="el-icon-delete table-operation" :title="$t('common.delete')"
             style="color: #f50;" v-hasPermission="['group:delete']"/>
          <el-link class="no-perm" v-has-no-permission="['group:update', 'group:copy', 'group:delete']">
            {{ $t("tips.noPermission") }}
          </el-link>
        </template>
      </el-table-column>
    ...
    <group-edit :dialog-visible="dialog.isVisible" :type="dialog.type"
            @close="editClose" @success="editSuccess" ref="edit"/>
    <group-import ref="import" :dialog-visible="fileImport.isVisible" :type="fileImport.type"
  ...
  data() {
    return {
      ...
      fileImport: {
        isVisible: false,
        type: "import",
        action: `${process.env.VUE_APP_BASE_API}/ucenter/group/import`
      },
      tableKey: 0,
