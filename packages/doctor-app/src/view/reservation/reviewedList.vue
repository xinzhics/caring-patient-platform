<template>
  <div style="min-height: 100vh;background: #F5F5F5;position: relative">
    <x-header style="margin-bottom:0px !important" :left-options="{backText: '',preventGoBack:true}"
              @on-click-back="goBack">待审核
      <a slot="right" v-if="dataList.length>0" class="fa fa-exist">
        <img @click="showClearExpirationDilog" src="@/assets/my/brush.png" width='20px' alt="">
      </a>
    </x-header>
    <div v-if="showNoDataImg&&dataList.length<1" style="width: 152px;position: absolute;top: 50%;left: 50%;transform: translate(-50%,-50%)">
    ...
    >
      <div v-for="(item,index) in dataList" :key="index" class="list">
        ...
        <div class="patient-info">
          <div style="margin-right: 10px;width: 43px;height: 43px;border-radius: 50%">
            <img style="border-radius: 50%" width="43px" height="43px" :src="item.avatar ? item.avatar : require('@/assets/my/touxiang.png')" alt="">
          </div>
          <div>
    ...
    >
      ...
      <div style="border-top:1px solid #EBEBEB;display: flex;justify-content: space-between;">
        <p style="padding:8px 0px;line-height:30px;text-align:center;color:#333;width:50%;border-right:1px solid #EBEBEB ;"
           @click="clearExpiration=false">取消</p>
        <p style="padding:8px 0px;line-height:30px;text-align:center;color:#333333;width:50%" @click="clearAppoint()">
          确定</p>
      </div>
    </van-dialog>
    <!--    拒绝弹窗-->
    ...
    >
      ...
      <div style="margin-bottom:10px">
        <p style="margin:0 0 5px;line-height:26px;text-align:center;color:#666">该时段已约满，是否增加1个号源
        </p>
        <p style="margin:0 0 20px;line-height:26px;text-align:center;color:#666">并通过审核</p>
      </div>
      <div style="border-top:1px solid #EBEBEB;display: flex;justify-content: space-between;">
        <p style="padding:8px 0px;line-height:30px;text-align:center;color:#333;width:50%;border-right:1px solid #EBEBEB ;"
           @click="isFull=false">取消</p>
        <p style="padding:8px 0px;line-height:30px;text-align:center;color:#333333;width:50%" @click="addSource()">
          确定</p>
      </div>
    </van-dialog>
  </div>
...
import Vue from 'vue';
import {Button, Field, List} from 'vant';
  ...
  methods: {
    goBack() {
      ...
      if (window.history.length>1) {
        this.$router.replace('/reservation/index')
      } else {
        wx.closeWindow()
      }
    },
    jumpChat(item) {
      ...
      doctorApi.getContentPatient(params).then(res => {
        if (res.data.code === 0) {
          this.$router.push({
            path: '/im/index', query: {
              imAccount: res.data.data.imAccount,
    ...
    // 增加号源
    addSource() {
      Api.addDirectApproval(this.appointmentId).then(res => {
        console.log(res)
        if (res.data.code === 0) {
          this.isFull = false
          this.$vux.toast.text('通过成功', 'cneter');
    ...
    refuse() {
      if (this.doctorApproveParams.auditFailReason==='OTHER'&&this.doctorApproveParams.auditFailReasonDesc === '') {
        this.$vux.toast.text('请输入拒绝原因', 'cneter');
        return
      } else  {
        Api.doctorApprove(this.doctorApproveParams).then(res => {
          console.log(res)
          if (res.data.code === 0) {
            this.dataList.splice(this.refuseIndex, 1)
            this.refuseDilog = false
            this.$vux.toast.text('拒绝成功', 'cneter');
    ...
    doctorApprove(item, index) {
      ...
      if (item.remainingNumber === 0) {
        ...
        Api.doctorApprove(params).then(res => {
          console.log(res)
          if (res.data.code === 0) {
     ...
     */
    clearAppoint() {
      Api.clearAppoint(localStorage.getItem('caring_doctor_id')).then(res => {
        console.log(res)
        ...
        if (res.data.code === 0) {
          ...
          this.$vux.toast.text('清除成功', 'cneter');
    ...
    getApproval() {
      if (this.loading && this.showNoDataImg) {
        return
      }
      this.loading = true
      Api.approval(this.params).then(res => {
        if (res.data.code === 0) {
