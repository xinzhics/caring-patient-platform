...
export default {
  ...
  methods: {
    doctorQueryCountOtherPush(followTaskContentId) {
      ...
      Api.doctorQueryCountOtherPush(followTaskContentId)
        .then(res => {
    ...
    sendChart() {
      let option = {
        ...
        series: [
          ...
          },
          {
            ...
            data: [
              { value: this.dataInfo.openUserNumber, name: '已打开', itemStyle:{
      ...
      var sendChart = echarts.init(document.getElementById("send"));
      // 绘制饼图
          ...
          },
          {
            ...
            data: [
              { value: this.dataInfo.completeNumber, name: '已完成', itemStyle:{
                  color: new echarts.graphic.LinearGradient(0.5, 0.5, 1, 0.5, [
                    {offset: 0, color: '#60E2B5'},
                    {offset: 1, color: '#4BC198'}
                  ]),borderRadius:50,onZeroAxisIndex:999} },
