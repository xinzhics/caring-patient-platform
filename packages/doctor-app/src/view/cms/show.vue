<template>
  <section>
    ...
    <div class="tipCover" v-if="caring_doctor_id">
      <div v-if="isSend">
...
export default {
  ...
  mounted() {
    ...
    if (that.$route.query && that.$route.query.id) {
      ...
      if (that.$route.query.isComment) {
        that.isComment = false
      }
      that.getInfo()
      this.setgood()
    } else if (that.$route.params && that.$route.params.id) {
  ...
  data() {
    return {
      ...
      caring_doctor_id:localStorage.getItem('caring_doctor_id')
    }
  },
  methods: {
    onCommit() {
    ...
    getwxSignature() {
      ...
      Api.wxAnnoSignature(params).then(res => {
        if (res.data.code === 0) {
          wx.config({
          ...
          wx.ready(function (res) {
            wx.updateAppMessageShareData({
              ...
              link: that.$getWxConfigSignatureUrl(), // 分享链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致
