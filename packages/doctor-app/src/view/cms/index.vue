<template>
  <section>
    <x-header style="margin-bottom:0px !important" :left-options="{backText: '',showBack:showback}">医生学院
      <a v-if="caringDoctorId" slot="right" class="fa fa-exist"><img style="width: 1.2rem;" :src="save" alt=""
                                               @click="$router.push('/cms/save')"></a>
    </x-header>
    <div style="padding-top: 50px;">
      <swiper :list="baseList" auto show-desc-mask v-model="indexNum"></swiper>
      <tab :line-width=2 active-color='#119CFF' v-model="indexTab">
        <tab-item class="vux-center" :selected="demo2 === item.id" v-for="(item, index) in list2"
                  @click.native="click(item.id)" :key="index">{{item.channelName}}
        </tab-item>
      </tab>
    </div>
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      if (this.$route.query.doctorOwner) {
        this.params.model.doctorOwner = Number(this.$route.query.doctorOwner)
      }
      if (this.$route.query.channelGroupId && this.$route.query.channelGroupId !== 'undefined' && this.$route.query.channelGroupId !== undefined) {
        this.params.model.channelGroupId =  this.$route.query.channelGroupId
      }
      if (!this.params.model.doctorOwner && !this.params.model.channelGroupId) {
        this.params.model.doctorOwner = 1
      }
    ...
    methods: {
      ...
      getChannelGroup() {
        if (this.params.model.channelGroupId) {
          Api.getChannelGroup(this.params.model.channelGroupId).then(res => {
            if (res.data.code === 0) {
              if (res.data.data && res.data.data.groupName) {
      ...
      getChannelList() {
        ...
        Api.channelpage(params).then((res) => {
          if (res.data.code === 0) {
            const thisObj = {
              channelName: '全部'
            }
