        ...
        <van-pull-refresh v-model="refreshing" @refresh="onRefresh" style="background-color: #f5f5f5"
          ...
          <van-list
            ...
            <div v-for="(item, key) in list" :key="key"
              ...
              <!-- ai消息 -->
              <div v-else-if="item.senderRoleType != 'user'"
...
import chatApi from '@/api/baiduChat.js'
...
export default {
  ...
  computed: {
    ...mapGetters({
      list: 'onBaiDuAiMsgList',
      msgPos: 'onBaiDuMsgPos',
      aiMsgStatus: 'onBaiDuAiMsgStatus',
    }),
  },
  mounted() {
    ...
    Vue.$store.commit('baiDuAiMsgStatus', {
      aiMsgStatus: true
    })
  },
  methods: {
    ...
    //点击其他地方关闭弹窗
    closePopover() {
      if (this.list[this.indexPopover] && this.list[this.indexPopover].showPopover) {
    ...
    createSse() {
      ...
      let url = baseUrl + '/msgs/baiduBotDoctorChat/createSse?uid=' + uid;
      ...
      eventSource.onmessage = (event) => {
        ...
        if (flag) {
          // 如果是新消息，需要添加到消息列表里面
          Vue.$store.commit('updateBaiDuAiMessage', {
            message: {
              ext: {
                content: text,
                senderRoleType: 'Ai',
          ...
          // 不是新消息，则需要更新文本内容
          Vue.$store.commit('updateCurrentBaiDuAiMessage', {
            message: {
              ext: {
                content: text,
              }
      ...
      chatApi.baiduBotSseChat(params)
        .then(res => {
    ...
    // 获取消息列表
    getAiChatListPage() {
      Vue.$store.commit('updateBaiDuMsgPos', {
        pos: 0
      })
      chatApi.getAiChatListPage(this.list && this.list.length > 0 ? this.list[0].createTime : moment().format("yyyy-MM-DD HH:mm:ss.SSS"))
        .then(res => {
          ...
          Vue.$store.commit('setBaiDuAiMessageList', {
    ...
    getBaiDuLastNewMessage() {
      ...
      chatApi.getBaiDulastNewMessage()
        .then(res => {
          if (res.data.data) {
            Vue.$store.commit('setBaiDuAiMessageList', {
    ...
    getSystemContent(content) {
      if (content != '0') {
        return content
      } else {
        return '你好，我可以基于输入的症状、体征等描述，推荐疑似诊断、建议检验检查。请问我有什么可以帮到你的？'
      }
    },
    // 获取医生信息
    getContent() {
