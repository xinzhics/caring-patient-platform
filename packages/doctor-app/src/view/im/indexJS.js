...
export default {
  ...
  data() {
    return {
      ...
      startTime: '',
      endTime: '',
      appUserCall: this.$getDictItem('assistant'), //医助
      wxUserCall: this.$getDictItem('patient'),
      doctorName: this.$getDictItem('doctor'),
      isSend: false,
      timeOutEvent: 0,
      imAccount: localStorage.getItem('userImAccount'),
      independence: 0, //1 为独立医生， 0 为非独立医生
      title: '健康服务小组',
      headerImg: require('@/assets/my/im_group_open.png'),
      imGroupStatus: 1, //1是展开聊天组头像     0为不展开
      isDrawer: false,
      drawerImg: require('@/assets/my/im_add.png'),
  ...
  updated() {
    if (this.updateMessage) {
      ...
      Vue.$store.commit('setLoading', {loading: false})
    }
  },
  mounted() {
  ...
  methods: {
    getRecommendationName(params) {
      if (JSON.parse(params).name) {
    ...
    onClickText(item) {
      ...
      setTimeout(() => {
        if (item.recommendationFunction === "CALENDAR") {
          // 用药日历
          this.$router.push({
            path: '/mypatientHome',
            query: {id: this.patientInfo.id, imMessageId: item.id, dictItemType: item.recommendationFunction}
          })
        ...
        } else if (item.recommendationFunction === "TEST_NUMBER") {
          // 检验数据
    ...
    imRecommendationHeat(data) {
      DoctorApi.imRecommendationHeat(data)
        .then(res => {
          this.getDoctorImRecommend(data)
        })
    },
    // 推荐功能发送消息
    sendRecommendIm(item) {
      if (this.doctorExitChat === 1) {
        this.isImOpenDialog = true
      }
      ...
      if (item.dictItemType === 'CUSTOM_FOLLOW_UP') {
        ...
        DoctorApi.planDetail(planId).then(res => {
          if (res.data.code === 0 && res.data.data) {
            ...
            Api.sendMessage(params)
              .then(res => {
                ...
                Vue.$store.commit('updateMyMessage', {
                  message: {
                ...
                console.log('==========', recommendationFunctionParams)
                this.scollBottom()
                that.isSend = false
              })
    ...
    getDoctorImRecommend() {
      DoctorApi.getDoctorImRecommend()
        .then(res => {
    ...
    goBack() {
      //
      if (this.$route.query && this.$route.query.formPage === 'weiXinTemplateMessage') {
    ...
    getChatContent(item) {
      if (this.doctorId === item.senderId) {
        return '您已' + (item.type === 'exitChat' ? '退出' : '加入') + '聊天'
      } else {
        ...
        .then(res => {
          ...
          this.$store.commit('updateMyMessage', {
            message: {
              ...
              content: '',
              senderId: localStorage.getItem('caring_doctor_id'),
              type: chatStat === 0 ? 'openChat' : 'exitChat',
              senderAvatar: localStorage.getItem('avatar'),
              senderRoleType: 'doctor',
              senderImAccount: this.imAccount
            }
          ...
          setTimeout(() => {
            ...
            this.getPatientDoctorInfo(true)
            this.scollBottom()
          }, 500)
        })
    },
    ...
    // 点击其他地方关闭弹窗
    closePopover() {
      if (this.messageList && this.messageList[this.indexPopover] && this.messageList[this.indexPopover].showPopover) {
        let item = this.messageList[this.indexPopover]
        item.showPopover = false
        this.messageList.splice(this.indexPopover, item)
      }
    },
    // 消息内容页面被触屏 并 移动。 猜测可能是用户在滑动屏幕 。此时收起软键盘
          ...
          .then(() => {
            Api.deleteStatusChat(this.messageList[index].id)
              .then(res => {
                ...
                Vue.$store.commit('deleteMessageList', {
                  index: index
                })
              })
          })
      ...
      this.timeOutEvent = setTimeout(function () {
        ...
        if (type === 'my') {
          ...
          if (that.differenceTime(moment(item.createTime).format('YYYY-MM-DD HH:mm:ss'), moment().locale('zh-cn').format('YYYY-MM-DD HH:mm:ss')) <= 120) {
            if (item.type === 'text' && !item.recommendationFunction) {
    ...
    popoverGotouchend(item, index, type) {
      ...
      if (!this.messageList[index].showPopover && this.isPopoverClick) {
        ...
        if ('voice' === item.type) {
          ...
          let leftParams = {
            direction: 'Previous',
            patientImAccount: this.$route.query.imAccount,
    ...
    computeHeight() {
      if (this.independence !== 0) {
        if (this.isDrawer) {
          if (this.imGroupStatus === 1) {
            return 'height: calc(100vh - ' + (180 + this.footerHeight) + 'px)'
          } else {
            return 'height: calc(100vh - ' + (90 + this.footerHeight) + 'px)'
          }
        } else {
          if (this.imGroupStatus === 1) {
            return 'height: calc(100vh - ' + (180 + this.footerHeight) + 'px)'
        ...
        return 'height: calc(100vh - ' + (70 + this.footerHeight) + 'px)'
      }
    },
    ...
    //手释放，如果在500毫秒内就释放，则取消长按事件，清除timeout
    gotouchchendImage(item) {
      clearTimeout(this.timeOutEvent)
      if (this.isHeaderImage && item.senderId === localStorage.getItem('patientId')) {
    ...
    uploadRecording(id) {
      ...
      let time = this.differenceTime(this.startTime, this.endTime)
      if (time > 0) {
        wx.uploadVoice({
    ...
    uploadServerId(serverId, time) {
      Api.uploadServerId({mediaId: serverId, wxAppId: localStorage.getItem('wxAppId')}).then(response => {
    ...
    // 跳转到患者详情
    JumpPatient(item) {
      if (this.patientInfo && item.type === 'patient') {
        this.$router.push({path: '/mypatientHome', query: {id: this.patientInfo.id}})
      }
    },
    // 跳转到小组详情
    JumpDetail() {
      if (this.patientInfo) {
        this.$router.push({path: '/im/messageDetail', query: {id: this.patientInfo.id}})
      }
    },
    // 文本发送
    inputSend() {
      ...
      Api.getMessageList(params)
        .then(response => {
          if (response.data.data.length > 0) {
            Vue.$store.commit('onMessageList', {
          ...
          if (!upload) {
            ...
            this.timeOutEvent = setTimeout(function () {
              ...
              let cms = that.$route.query.cms
              if (cms) {
                let im = that.$route.query.imAccount
                let id = that.$route.query.imPatientId
                that.onSendTextMsg('cms', cms)
                that.$router.replace({
                  query: {
                    imAccount: im,
                    imPatientId: id,
                    formPage: that.$route.query.formPage ? that.$route.query.formPage : ''
                  }
                })
              } else {
                if (that.isApple) {
    ...
    updateMessageState() {
      const msgParams = {
        groupIm: this.patientInfo.imAccount
      }
    ...
    // im登录
    webIMLogin() {
      DoctorApi.getContent({id: localStorage.getItem('caring_doctor_id')}).then(res => {
        if (res.data.code === 0) {
          this.imAccount = res.data.data.imAccount
          const userName = this.imAccount
    ...
    // 获取患者医生信息
    getPatientDoctorInfo(flag) {
      //获取服务器IM历史聊天页面
      const params = {
        patientId: this.$route.query.imPatientId
      }
      Api.getPatientDoctorInfo(params).then(res => {
        if (res.data.code === 0) {
          this.$store.commit('updatePatient', {
            id: this.$route.query.imAccount
          })
          this.independence = res.data.data.independence //1 为独立医生， 0 为非独立医生
          //获取患者信息，判断是否隐藏三人头像
          Api.getContent(params).then(pRes => {
            this.patientInfo = pRes.data.data
            if (this.independence === 0) {
              //为非独立医生需要设置聊天标题
              if (this.patientInfo.doctorId === localStorage.getItem('caring_doctor_id')) {
                this.title = this.patientInfo.name + '(我的' + this.wxUserCall + ')'
              } else {
                this.title = this.patientInfo.name + '( ' + this.patientInfo.doctorName + '的' + this.wxUserCall + ')'
              }
              ...
              //独立医生保持原来的界面
              this.imGroupStatus = pRes.data.data.doctorImGroupStatus ? pRes.data.data.doctorImGroupStatus : 0
              this.headerImg = this.imGroupStatus === 1 ? require('@/assets/my/im_group_close.png') : require('@/assets/my/im_group_open.png')
                  ...
                  response.data.data.forEach(item => {
                    ...
                    if ('NursingStaff' === item.type) {
                      ...
                      this.popupList.push({
                        id: item.id,
                        type: 'patient',
                        name: item.doctorRemark ? item.doctorRemark : item.name,
                        avatar: item.avatar
                      })
                      this.doctorExitChat = item.patient.doctorExitChat
                      patient = item
                    }
                  })
                  this.patientImGroup = []
                  if (patient.patient.doctorExitChat == 1 && patient.patient.nursingExitChat == 1) {
                    // 医生医助都退出
                    this.patientImGroup.push(patient)
                  } else if (patient.patient.doctorExitChat === 1 || patient.patient.nursingExitChat === 1) {
                    response.data.data.forEach(item => {
                      if ("patient" === item.type) {
                        this.patientImGroup.push(item)
                      } else if ("doctor" === item.type && patient.patient.doctorExitChat !== 1) {
                        this.patientImGroup.push(item)
                      } else if ("NursingStaff" === item.type && patient.patient.nursingExitChat !== 1) {
                        this.patientImGroup.push(item)
                      }
                    })
                  } else {
          ...
          this.$toast.fail(res.data.msg, 'cneter');
          setTimeout(() => {
            wx.closeWindow()
          }, 2000)
    ...
    //群组成员获取
    getGroupDetail() {
      Api.getPatientGroupDetail(this.patientInfo.id).then(res => {
        if (res.data.code === 0) {
          ...
          this.popupList.push({
            id: res.data.data.patient.id,
            type: 'patient',
            name: res.data.data.patient.doctorRemark ? res.data.data.patient.doctorRemark : res.data.data.patient.name,
            avatar: res.data.data.patient.avatar
          })
    ...
    isImGroup() {
      const params = {
        id: this.$route.query.imPatientId,
        doctorImGroupStatus: this.imGroupStatus === 1 ? 0 : 1
      }
      Api.isImGroup(params).then(res => {
        ;(this.imGroupStatus = this.imGroupStatus === 1 ? 0 : 1),
          (this.headerImg = this.imGroupStatus === 0 ? require('@/assets/my/im_group_open.png') : require('@/assets/my/im_group_close.png'))
      })
    },
    setIsApple() {
      var u = navigator.userAgent;
  ...
  beforeDestroy() {
    Vue.$store.commit('updatePatient', {
      id: ''
    })
  ...
  created() {
    // 医生
    this.getwxSignature()
    this.getDoctorImRecommend()
    this.setIsApple()
    this.receiverImAccount = this.$route.query.imAccount
    this.webIMLogin()
    this.getPatientDoctorInfo()
    if (this.$route.query.isAt && this.$route.query.isAt === true) {
      Dialog.alert({
        title: '提示',
