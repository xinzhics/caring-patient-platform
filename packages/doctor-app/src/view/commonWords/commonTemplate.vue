<template>
  <section class="allContent">
    <van-sticky>
      <nav-bar v-if="!searchModel" pageTitle="常用语模板库" @clickBack="back()" :back-function="true"></nav-bar>
    ...
    <round-dialog ref="rDialog" @selectConfirm="selectDialogConfirm"
...
<script>
import Vue from 'vue';
import { Tab, Tabs, Search, Icon, Toast, Sticky } from 'vant'
...
export default {
  ...
  data() {
    return {
      ...
      params: {
        ...
        size: 10,
        model: {
          remarkUserSelect: localStorage.getItem('caring_doctor_id'),
          userType: 'doctor',
          templateTypeId: '',
          templateRelease: 1,
        }
  ...
  created() {
    ...
    this.params.model.remarkUserSelect = localStorage.getItem('caring_doctor_id')
    this.onRefresh()
  ...
  methods: {
    ...
    importTemplateSubmit() {
      ...
      CommonApi.importTemplateCommon(this.doctorId, 'doctor', this.selectImportTemplateIds).then(res => {
        if (res.data.code === 0) {
          Toast({
            duration: 1000, // 持续展示 toast
            closeOnClick: true,
    ...
    getComMsgTemplateList() {
      ...
      CommonApi.getComMsgTemplateList(params)
        .then(res => {
    ...
    getComMsgTemplateType() {
      ...
      CommonApi.getComMsgTemplateType(data)
        .then(res => {
