<template>
  <div style="height: 100%; width: 100%; margin: 0 auto" class="messagebox">
    <x-header :left-options="{backText: ''}">{{pageTitle ? pageTitle : '小组'}}
              ...
              <div style="display: flex; float: left;">
                <div>
                  ...
                  display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp:1; overflow: hidden">{{
                      patientInfo && patientInfo.doctorRemark && item.senderRoleType === 'patient' ? patientInfo.doctorRemark : item.senderName }}
                  </div>
                </div>
    ...
    <div class="messagebox-footer" ref="footer">
      <van-row style="width: 100%; height: 100%; display: flex; align-items: center">
        ...
        <van-col span="3"
          ...
          <img :src="$data.message == '' ? require('@/assets/my/send.png') : require('@/assets/my/blue_send.png')"
               @click="onSendTextMsg('text', '')"
               width="25px" height="25px">
        </van-col>
      </van-row>
...
export default {
  ...
  updated() {
    if (this.updateMessage) {
      this.scollBottom();
      Vue.$store.commit("setGroupLoading", {loading: false});
    }
  },
  beforeDestroy() {
  ...
  methods: {
        ...
        if (response.data.data.length > 0) {
          ...
          if (!this.patientInfo.name) {
            ...
            response.data.data.forEach(item => {
              if (item.senderRoleType === 'patient' && sendPatient.length < 1) {
                sendPatient.push(item)
              }
            })
            if (sendPatient.length > 0) {
              doctorApi.getContentPatient({id:sendPatient[0].createUser}).then(res => {
                if (res.data.code === 0) {
          ...
          Vue.$store.commit("onGroupMessageList", {
    ...
    jumpClick() {
      this.$router.push({path:'/consultation/detail', query: {remark: this.patientInfo.remark}})
    },
    //发送文本信息
    ...
    updateMessageState() {
      ...
      doctorApi.doctorReadMessage(msgParams)
        .then((el) => {
      })
    }
  },
  ...
  created() {
    this.imAccount = localStorage.getItem('userImAccount');
