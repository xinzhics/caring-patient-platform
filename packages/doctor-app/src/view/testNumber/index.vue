<template>
  <section class="allContent">
    <x-header :left-options="{backText: ''}">{{pageTitle ? pageTitle : alldata.name}}
      <img slot="right" @click="$router.push('/testNumber/editor')" style="width:20px" :src="addImg" alt="" srcset="">
    </x-header>
    <div v-if="allData.length == 0">
      <div class="nodata">
        <img :src="noData" alt="" style="padding-top: 150px;">
        <p>暂未添加{{pageTitle ? pageTitle : alldata.name}}</p>
      </div>
    </div>
                      ...
                      <flexbox>
                        <flexbox-item v-for="(i, k) in getImage(item.values)" :key="k" v-if="k < 4">
                          <div class="flex-demo" v-if="i.attrValue">
                            <van-image v-if="k < 3" width="4rem" height="4rem" style="margin-right: 10px;margin-top: 10px;" :src="i.attrValue" />
