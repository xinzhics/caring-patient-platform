<template>
  <section class="main">
    <x-header style="margin-bottom:0px!important;" :left-options="{backText: ''}">{{ pageTitle ? pageTitle : '智能提醒' }}
    </x-header>
    ...
    Api.getH5UIQuery()
      .then(res => {
        if (res.data && res.data.code === 0) {
          ...
          this.imageUrl = res.data.data.NURSING_BG.attribute1
        }
      })
  },
  methods: {
    ...
    getInfo() {
      ...
      Api.getMyNursingPlans(params).then((res) => {
        if (res.data.code === 0) {
          // res.data.data.subscribeList.forEach(el => {
          //     el.isSubscribe = el.isSubscribe===1?true:false
          // });
    ...
    submit() {
      ...
      // params.subscribeList.forEach(el => {
      //     el.isSubscribe = el.isSubscribe?1:0
      // });
      const param = {
        patientId: localStorage.getItem('patientId'),
        content: params
