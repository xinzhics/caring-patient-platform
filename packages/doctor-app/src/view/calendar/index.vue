<template>
  <section class="main">
    <x-header :left-options="{backText: ''}">{{pageTitle ? pageTitle : alldata.name}}<a slot="right" class="fa fa-exist"><img style="width: 1.3rem;"
                                                                                          :src="headerImg" alt=""
                                                                                          @click="$router.push('/medicine/index')"></a>
    </x-header>
          ...
          <section class="day">
            ...
            <span class="dayBar" v-for="(day, index ) in dayCalender" :key="'dayCalender' + index"
                  @click="clickDayDetail(day)">
                    <div :class="{'item':true}">
                        <div
                          :class="{itemInner:true, 'smallchange': new Date().toDateString()==new Date(day.date).toDateString(), 'bigchange': choseFlag === day && new Date() > new Date(choseFlag.date)} ">
                            <span
    ...
    <div style="background: #f5f5f5" v-if="list && list.length !== 0">
      <div style="margin-top: 15px; background: #FFF;display: block; padding: 15px;">
        ...
        <div style="display: flex; align-items: center; overflow:hidden;position: relative;" v-for="(i, k) in list" :key="k">
          ...
          <!--margin-bottom: -10000px; margin-top: -10000px; padding-top: 10000px; padding-bottom: 10000px; 填充布局-->
          <div
            style="height: 100%; flex: 1; margin-left: 15px; margin-right: 20px; margin-bottom: -1000px; margin-top: -1000px; padding-top: 1000px; padding-bottom: 1000px; position: relative">
            <!-- <div

              style="height: 100%; width: 2px; margin-bottom: -1000px; margin-top: -1000px; padding-top: 1000px; padding-bottom: 1000px; background: #E6E6E6;">
            </div> -->
            <div
          ...
          <flexbox
            style="background: #f5f5f5; border-radius: 10px; padding: 10px; margin-top: 10px; margin-bottom: 10px;">
            <flexbox-item :span="9">
            ...
            <flexbox-item :span="3" style="display: flex; justify-content: center;">
...
export default {
  ...
  data() {
    return {
      bloodimg: require('@/assets/my/calender.png'),
      ...
      medicationDay: '',
      list: [],
      //   showMonth:new Date().toLocaleDateString().split('/').join('-')
    }
  },
  // created() {
  //   this.initialDateSet()
  ...
  methods: {
    ...
    getMedicationPlan() {
      Api.getPatientMedicationPlan(this.medicationDay)
        .then(res => {
