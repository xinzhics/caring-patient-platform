  ...
  <section v-if="canshow">
    ...
    <div class="medicine-div-list" :style="{height: height + 'px'}">
      ...
      <div v-if="scanMedicineState && !loading">
        <p v-if="medicineList && medicineList.length > 0" style="padding:10px 16px 0px">扫描结果</p>
      ...
      <div v-if="searchMedicineState && !loading">
        ...
        <div v-if="current < pages" style="text-align:center;line-height:30px;font-size:16px;padding:5px 0px 25px"
             @click="current = current+1; getMoreMedicine()">
          <span>点击加载更多...</span>
        </div>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      noData: require('@/assets/my/noData.png'),
  ...
  methods: {
    ...
    getMedice(k) {
      this.inputVal = ''
      this.$refs.medicineNavBar.clearInput()
      this.loading = true
      this.medicineList = []
      this.searchMedicineState = false
