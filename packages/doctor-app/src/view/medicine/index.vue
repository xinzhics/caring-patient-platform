<template>
  <section class="main">
    <navBar @toHistoryPage="$router.push('/medicine/history')"
            :rightIcon="dateImg" :showRightIcon="true" backUrl=""
            :pageTitle="'我的药箱'"/>
          ...
          <van-swipe-cell @open="openSwipe('current', index)" @close="closeSwipe('current', index)"
            ...
            <div class="box" :style="{ borderRadius: openCurrentIndex === index ? '0px' : '10px' }">
              <div style="width: 80%" @click="goItem(item)">
                ...
                <div style="font-size: 13px; color: #333; margin-top: 10px;">
                  {{ item.cycle === 0 ? '长期' : item.cycleDay + '天' }}
                  、{{ item.cycleDuration + getFrequency(item.timePeriod) + item.numberOfDay + '次' }}
                </div>
                <div style="display: flex; flex-flow: row wrap; align-items: flex-start;">
                  <div v-for="(itemDate, key) in item.patientDrugsTimeSettingList" :key="key"
                    ...
                    {{ itemDate.dayOfTheCycle ? '第' + itemDate.dayOfTheCycle + '天' : '' }}
            ...
            <template slot="right">
              <van-button square type="danger" text="删除" @click="deleteItem(item)"
                          style="height: 100%; border-top-right-radius: 10px; border-bottom-right-radius: 10px;"/>
            </template>
          </van-swipe-cell>
        </div>
      </div>
    ...
    <div class="noData" v-if="currentList.length === 0 && historyList.length === 0 && !isLoading">
      ...
      <x-button style="background:#66728C;width:40%;color:#fff;margin:15vw auto 0px;"
                @click.native="$router.push('/medicine/addmedicine')">添加用药
      </x-button>
    </div>
...
export default {
  ...
  methods: {
    ...
    historySwitch(i) {
      ...
      data.status = 0
      Api.FixpatientDrugs(data).then((res) => {
        if (res.data.code === 0) {
          this.getData()
        }
      })
