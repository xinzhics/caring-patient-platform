...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      ContentApi.getPatientMonitoringDataPlan(this.patientId)
        .then(res => {
    ...
    cancelBtn(item, index) {
      ContentApi.patientMonitoringCancelPlan(this.patientId, item.planId, item.planType)
        .then(res => {
          if (res.data && res.data.code === 0) {
            this.getData()
          }
    ...
    // 关注
    subscribeBtn(item) {
      ContentApi.setPatientMonitoringSubscribePlan(this.patientId, item.planId, item.planType)
        .then(res => {
          if (res.data && res.data.code === 0) {
            this.getData()
          }
