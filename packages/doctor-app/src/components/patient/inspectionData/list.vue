    ...
    <div style="height: auto;">
      <van-pull-refresh v-model="refreshing" @refresh="onRefresh"
        ...
        <van-list
          ...
          finished-text="没有更多了"
          @load="onLoad">
          <group v-if="list.length > 0" v-for="(i,k) in list" :key="k">
            <van-swipe-cell>
...
import {SwipeCell, Button, Empty, Dialog} from 'vant';
...
export default {
  ...
  methods: {
    ...
    add() {
      this.$router.push('/testNumber/editor')
    },
    putOrDeletePlan() {
    ...
    onLoad() {
      ...
      ContentApi.getCheckData(params).then((res) => {
        if (res.data.code === 0) {
    ...
    putPatientSubscribePlanFunction() {
      ContentApi.putPatientSubscribePlan(this.patientId, null, this.planType).then(res => {
        if (res.data.code === 0) {
          this.getPatientSubscribePlanFunction()
        }
      })
    ...
    deletePatientSubscribePlanFunction() {
      ContentApi.deletePatientSubscribePlan(this.patientId, null, this.planType).then(res => {
        if (res.data.code === 0) {
          this.getPatientSubscribePlanFunction()
        }
      })
    ...
    getPatientSubscribePlanFunction() {
      ContentApi.getPatientSubscribePlan(this.patientId, null, this.planType).then(res => {
        if (res.data.code === 0) {
    ...
    deleteFormResult(id) {
      ContentApi.deleteFormResult(id).then((res) => {
        if (res.data.code === 0) {
          this.onRefresh()
        }
      })
