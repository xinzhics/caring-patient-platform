...
export default {
  ...
  methods: {
    ...
    beforeClose({ position, instance }) {
      switch (position) {
        case 'right':
          ContentApi.updateForDeleteFormResult(this.contentList[this.listIndex].id).then(res => {
            console.log(res)
            if (res.data.code === 0) {
    ...
    healthFormResultList() {
      ...
      this.loading = true;
      ContentApi.healthFormResultList(this.params).then(res => {
        if (res.data.code === 0) {
          this.loading = false
