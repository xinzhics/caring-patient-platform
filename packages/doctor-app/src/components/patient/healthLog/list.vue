    ...
    <div>
      <van-pull-refresh v-model="refreshing" @refresh="onRefresh"
        ...
        <van-list
          ...
          finished-text="没有更多了"
          @load="onLoad">
          <group v-if="list.length > 0" v-for="(i,k) in list" :key="i.id" style="padding: 0px 10px 5px 10px">
...
export default {
  ...
  methods: {
    ...
    putPatientSubscribePlanFunction() {
      ContentApi.putPatientSubscribePlan(this.patientId, this.planId, this.planType)
        .then(res => {
          if (res.data.code === 0) {
            this.getPatientSubscribePlanFunction()
          }
    ...
    deletePatientSubscribePlanFunction() {
      ContentApi.deletePatientSubscribePlan(this.patientId, this.planId, this.planType)
        .then(res => {
          if (res.data.code === 0) {
            this.getPatientSubscribePlanFunction()
          }
    ...
    getPatientSubscribePlanFunction() {
      ContentApi.getPatientSubscribePlan(this.patientId, this.planId, this.planType)
        .then(res => {
          if (res.data.code === 0) {
