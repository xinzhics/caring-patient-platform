<template>
  <div style="background: #FFFFFF; padding: 15px 18px; border-right: 8px;">
    <div v-for="(field, index) in fieldList" :key="field.id" style="min-height: 45px; justify-items: center; width: 100%">
      <!-- 处理特殊组件  -->
      <descindex :field="field" v-if="field.widgetType === Constants.CustomFormWidgetType.Desc"></descindex>
      <!-- 分割线组件 -->
      <split-lineindex v-else-if="field.widgetType === Constants.CustomFormWidgetType.SplitLine" :field="field"></split-lineindex>
      <!-- 用户输入值的组件 -->
      <div v-else style="width: 100%;">
        ...
        || field.widgetType === Constants.CustomFormWidgetType.DropdownSelect">
          <div v-if="field.values && field.values.length > 0">
            <div v-for="(val, index) in field.values" :key="val.attrValue">
              <div v-if="val.questions && val.questions.length > 0">
                <div class="caring-form-result"></div>
                <div v-for="(ques, quesIndex) in val.questions" :key="ques.id" style="min-height: 45px; justify-items: center">
                  <single-row-com :field="ques" @openImagePreview="openImagePreview"></single-row-com>
                  <div class="caring-form-result" v-if="quesIndex < val.questions.length - 1 && val.questions.length > 1 "></div>
                </div>
              </div>
            </div>
          </div>
      ...
      queryImageList(fieldList) {
        if (fieldList) {
          for (let i = 0; i < fieldList.length; i++) {
            ...
            if (field.widgetType === this.Constants.CustomFormWidgetType.Avatar
              || field.widgetType === this.Constants.CustomFormWidgetType.SingleImageUpload
            ...
            } else if (field.widgetType === this.Constants.CustomFormWidgetType.Radio
              || field.widgetType === this.Constants.CustomFormWidgetType.CheckBox
