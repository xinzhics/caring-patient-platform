...
export default {
  ...
  methods: {
    ...
    chooseHospitalOption2(val) {
      ...
      this.$emit("hospitalName", val.hospitalName);
      // this.field.values = [];
    ...
    pageProvince() {
      this.params.model = {};
      this.params.current = 1;
      Api.annoQueryProvince(this.params).then(res => {
    ...
    pageCity() {
      ...
      Api.annoQueryCity(this.params).then(res => {
        if (res.data && res.data.code === 0) {
          if (res.data && res.data.code === 0) {
            if (this.params.current === 1) {
    ...
    hospitalPage() {
      ...
      Api.annoHospitalPage(this.params).then(res => {
        if (res.data && res.data.code === 0) {
          if (this.params.current === 1) {
            this.list = res.data.data.records;
          } else {
          ...
          this.loading = false;
          if (res.data.data.records && res.data.data.records.length < 200) {
            this.finished = true;
          }
        }
      });
