  ...
  <div>
    <div v-for='(info,index) in questions' :key="index">
      ...
      <div v-else-if='info.widgetType === Constants.CustomFormWidgetType.Radio'>
        <newHisSingleChoice :field='info' :userRole="userRole" @noClick="clickForm" :isHealth="$route.query.isHealth"/>
      </div>

      <div v-else-if='info.widgetType === Constants.CustomFormWidgetType.CheckBox'>
        <newHisMultipleChoice :field='info' :userRole="userRole" @noClick="clickForm" :isHealth="$route.query.isHealth"/>
      </div>
      ...
      <div v-else-if='info.widgetType === Constants.CustomFormWidgetType.Date'>
        <date :field='info' :userRole="userRole" @noClick="clickForm" :isHealth="$route.query.isHealth"/>
      </div>
