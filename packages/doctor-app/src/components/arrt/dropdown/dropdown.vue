<template>
  <div class="main">
    ...
    <div :class="show === false ? 'showbox' : 'show'">
      <div class="show1" :style="{'border-bottom':index>=0 && index < field.options.length-1 ? '1px solid #D6D6D6 ':''}"
           @click="chooseBox(info, index)" v-for="(info,index) in field.options " :key="index">
        <div class="choose" style="width:92%" :class="optionSelected(info, index) ? 'choose2':''">
    ...
    <div class="userinfo">
      <van-field v-if="field.values[0].valueText === '其他' && field.hasOtherOption === 1 " v-model="field.otherValue"
                 show-word-limit :class="[otherValueFail === true ? 'cell2 cell3' : 'cell3']"
                 :rules="[{ required: field.otherEnterRequired === 1 }]" :maxlength="field.otherValueSize"
