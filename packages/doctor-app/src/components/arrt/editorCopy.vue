      ...
      <div v-for='(info,index) in currentFields' :key="info.id" v-if="
        ...
        <div>
          ...
          <div v-else-if='info.widgetType === Constants.CustomFormWidgetType.SingleLineText && info.exactType === Constants.FieldExactType.SCR'>
            <scr :field='info' style="pointer-events:none" :ccrField="ccrField" :gfrField="gfrField" :courseOfDiseaseField="courseOfDiseaseField" />
          </div>
          ...
          <splitLine v-else-if='info.widgetType === Constants.CustomFormWidgetType.SplitLine' :field='info'>style="pointer-events:none"</splitLine>
        </div>
      </div>
    </van-form>
  </div>
