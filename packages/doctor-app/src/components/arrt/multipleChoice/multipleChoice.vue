  ...
  <div>
    <div class="main">
      ...
      <van-cell is-link :ref="field.id" @click="showPopup" v-if="!noValues"
        :style="{'border':field.modifyTags ? '1px dashed #337EFF':'1px solid #D6D6D6;','background':field.modifyTags?'rgba(51, 126, 255, 0.05)':''}">
        <!-- 一般选项 -->
        <div class="title3" :style="{color:!field.isUpdatable || field.isUpdatable !== 1?'#c8c9cc':'#333'}" v-for='(info,index) in field.values' v-if="info.attrValue" :key="index">
          <div class="box"  v-if="info.valueText !== ''">
            <div :style="{background:!field.isUpdatable || field.isUpdatable !== 1?'#c8c9cc':'rgb(51, 51, 51)'}" class="yuan"></div>
      ...
      <van-popup v-model="show" closeable close-icon-position="top-right" position="bottom">
        ...
        <div class="choose" v-for='(info,index) in field.options' :key="index">
          ...
          <div class="input1" v-if="info.select">
            ...
            <van-field v-if="field.itemAfterHasEnter === 1 && info.attrValue !== '其他'" class="cell2" :error="info.error"
              rows="1" autosize type="textarea" v-model="info.valuesRemark" @input="(val) => fieldInput(val, info)"
              :placeholder="field.itemAfterHasEnterRequired === 1? '请输入(必填)' : '请输入(选填)'" show-word-limit
