...
WebIM.conn.listen({
  ...
  onTextMessage: function (message) {
    console.log(message)
    if (message.ext.senderRoleType === 'Ai') {
      //如是AI角色发回的消息，则需要发送到AI助手界面
      Vue.$store.commit("updateAiMessage", {message: message});
    }else {
...
// 检查 webrtc 是否已从 CDN 加载
if (typeof webrtc !== 'undefined') {
  WebIM.WebRTC = webrtc;
} else {
  console.warn('WebRTC 未正确加载，视频通话功能不可用.');
}

// 检查 emedia 是否已从 CDN 加载
if (typeof emedia !== 'undefined') {
  WebIM.EMedia = emedia;
} else {
  console.warn('EMedia 未正确加载，音视频功能不可用.');
}
