  ...
  <div>
      ...
      <div v-for='(info,index) in currentFields' :key="info.id" v-if="
        ...
        <div>
          ...
          <div
            ...
            <height :field='info' :heightHandle="heightHandle" :userRole="userRole" @noClick="clickForm"
                    :isHealth="$route.query.isHealth"/>
          </div>

          <div
            v-else-if='info.widgetType === Constants.CustomFormWidgetType.SingleLineText && info.exactType === "Weight"'>
            <weight :field='info' :weightHandle="weightHandle" :userRole="userRole" @noClick="clickForm"
                    :isHealth="$route.query.isHealth"/>
          </div>
          ...
          <div
            ...
            <scr :field='info' :scrHandle="scrHandle" :ccrField="ccrField" :gfrField="gfrField"
                 :courseOfDiseaseField="courseOfDiseaseField" :userRole="userRole" @noClikc="clickForm"
                 :isHealth="$route.query.isHealth"/>
          </div>

          <div v-else-if='info.widgetType === Constants.CustomFormWidgetType.Date'>
            <date1 :field='info' :ageHandle="ageHandle" :userRole="userRole" @noClick="clickForm"
                   :isHealth="$route.query.isHealth"/>
          </div>
          ...
          <div
            ...
            <gender :field='info' :genderHandle="genderHandle" :userRole="userRole" @noClick="clickForm"
                    :isHealth="$route.query.isHealth"/>
          </div>
    ...
    <van-col span='24' style='text-align: center;background-color: #fff;position: relative'>
      <van-col v-if="totalPage > 1" span='24' style="
        ...
        ">
        <van-button :loading="isLoading" loading-type="spinner" loading-text="正在提交" :class="[lastPage?'tijiao2':'']"
                    style="background-color: rgb(102, 114, 140) !important; border: unset !important; margin-right: 13px;margin-top: 13px;margin-bottom: 13px;"
                    round type='primary' class='attr-button btn tijiao' @click='saveCurrentFields'>{{
      ...
      <van-col v-if='totalPage === 1' span='24' style="background: rgb(255, 255, 255);padding-bottom: 8.9375rem ;">
        <van-button :loading="isLoading" loading-type="spinner" loading-text="正在提交"
                    style="border-radius: 42px;width: 13rem;background: rgb(102, 114, 139) !important; border: unset !important;"
                    @click='saveCurrentFields' class='attr-button btn' type='primary'>{{ myTitle ? myTitle : '提交' }}
        </van-button>
      </van-col>
      <van-col v-if='currentPage && currentPage.canBackPrevious === 1 && currentIndex > 1' span='24'
               style='text-align: left;padding-bottom: 13px;padding-top: 13px !important;padding-left: 13px;background: #F7F7F7;'>
        <van-button style="background: #fff !important;width: 35% ;" @click='previousPage' class='attr-button btn' round
                    type='default'>{{
