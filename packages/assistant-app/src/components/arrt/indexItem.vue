                ...
                <van-row v-for="(option,k) in info.options" :key="k">
                  <van-col span="24">
                    <van-radio class="radio-item-padding option-word-wrap" :name="option.id">
                      {{ option.attrValue }}
                      <!-- <template slot="icon" >
                        <i class="icon iconfont icondanxuanweixuan theme-color" v-if="!option.checked"></i>
                        <svg class="icon svg-icon" style="width: 16px; height: 20px" aria-hidden="true" v-else>
...
import {ImagePreview,Row,Col,Cell,Form,Button,Field,DropdownMenu, DropdownItem,RadioGroup, Radio,Checkbox, CheckboxGroup} from "vant";
export default {
  name: "index-item",
  props: {
    question: {
