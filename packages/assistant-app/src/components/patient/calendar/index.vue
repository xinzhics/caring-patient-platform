<template>
  <div style="position: relative; height: 100%; background: #f5f5f5">
    ...
    <van-dialog v-model="show" class="dialog-demo">
...
<script>
import { getPatientMedicationPlan, patientDayDrugsCalendar, patientDayDrugsTo7Day, queryByDrugsTimestamp, clockIn } from '@/api/drugsApi.js'
import Vue from 'vue'
import moment from 'moment'
import {Button, Step, Steps, Dialog, Toast} from 'vant'
...
export default {
  ...
  methods: {
    ...
    getDrugsTime (i) {
      ...
      queryByDrugsTimestamp(params).then(res => {
        if (res.code === 0) {
    ...
    // 获取用药列表
    getMedicationPlan () {
      getPatientMedicationPlan(this.weekCurrentDate).then(res => {
        if (res && res.code === 0) {
          if (res.data) {
            this.list = res.data
            this.stepActive = -1
    ...
    getWeekInfo () {
      ...
      patientDayDrugsTo7Day(patientId).then(res => {
        if (res.code === 0) {
