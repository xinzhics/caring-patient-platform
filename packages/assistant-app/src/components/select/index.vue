...
export default {
  ...
  computed: {
    showText: {
      ...
      get () {
        return this.resultData && typeof this.resultData === 'object' && !this.type
          ? this.resultData['text'] || this.resultData['valueText'] : !this.type
            ? this.resultData : this.type === 'date'
              ? this.resultData
                ? parseTime(this.resultData, '{y}-{m}-{d}') : '' : this.resultData
      }
    }
  },
  watch: {
