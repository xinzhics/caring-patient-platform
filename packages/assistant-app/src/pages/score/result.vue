  ...
  <section class="myCenter">
    ...
    <div class="headerContent" :style="{backgroundImage:'url(https://caring.obs.cn-north-4.myhuaweicloud.com/wxIcon/form_result_score_bg_img.png)'}">
      <div style="position: absolute; top: 40px; left: 20px;">
        ...
        <div style="font-size: 14px; color: white; font-weight: 500">此测试结果仅供参考，不能代替医生诊断</div>
      </div>
      ...
      <div class="content">
        <div class="box" v-if="scoreRule.showResultSum === 1">
          ...
          <div class="value" v-if="scoreRule.formResultCountWay === 'average_score'">
            {{ data.formResultAverageScore }}分
          </div>
...
import { getFormResultScore, getPatientNewHomeRouter } from '@/api/patient.js'
import Vue from 'vue'
...
export default {
  ...
  data () {
    return {
      ...
      list: [],
      data: {},
      backQuery: {planId: this.$route.query.planId, title: this.$route.query.title},
  ...
  methods: {
    getPatientHome () {
      getPatientNewHomeRouter().then(res => {
        if (res.code === 0) {
          this.h5UiImage = res.data.h5UiImage
        }
      })
    },
    jump () {
    ...
    getFormResultScore () {
      getFormResultScore(this.$route.query.dataId)
        .then(res => {
          if (res.data) {
