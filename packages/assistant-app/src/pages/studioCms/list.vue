          ...
          <div class="cms-info">
            ...
            <div v-else class="cms-content_video">
              ...
              <div class="cms-content_video_text">
                <div class="cms-content_video_title">{{ cms.cmsFileTitle }}</div>
                <div class="cms-content_video_dsc">{{ cms.cmsFileType }} {{ formatFileSize(cms.cmsFileSize) }}</div>
              </div>
            </div>
...
import { Toast, Dialog, Overlay, Search, NavBar, List } from 'vant'
  ...
  methods: {
    ...
    queryCmsList () {
      ...
      getCmsList(this.pageParams).then(res => {
        ...
        if (res.data.pages <= this.pageParams.current) {
    ...
    toArticleImport () {
      this.$router.push({ path: '/studio/cms/articleImport',
        query: {
          articleUserId: this.articleImportUserId,
          doctorId: this.doctorId
        }
      })
    },
...
.empty-add-btn :deep(.van-button__content) {
  display: flex;
  align-items: center;
  justify-content: center;
  gap: 8px;
