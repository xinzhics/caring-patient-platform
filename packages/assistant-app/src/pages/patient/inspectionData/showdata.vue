<template>
  <section class="main">
    <van-sticky>
      <headNavigation leftIcon="arrow-left" title="检验数据详情" @onBack="back"></headNavigation>
    </van-sticky>
    <div class="box">
      <div class="label">
        <div class="strip"></div>
    ...
    <div style="display: flex; justify-content: center; padding: 25px 0px; width: auto">
      <van-button type="primary"
                  style="width: 90%;  margin-bottom: 15px; background: #67E0A7; border: 1px solid #67E0A7"
                  round @click="edit()">编辑数据
      </van-button>
    </div>
  </section>
</template>
...
import {patientDetail} from '@/api/patient.js'
import {Toast, Grid, GridItem, ImagePreview, Button, Image as VanImage} from 'vant'
    ...
    getData () {
      ...
      getCheckDataformResult(params).then(res => {
        ...
        if (res.code === 0) {
          ...
          let list = JSON.parse(res.data.jsonContent)
          list.forEach(item => {
            // 多图上传
          ...
          console.log('============', this.imageData.list)
        }
      })
    },
    getPatientInfo (userId) {
      patientDetail(userId).then((res) => {
        if (res.data) {
