          ...
          <span class="dayBar" v-for="(day, index ) in dayCalender" :key="'dayCalender' + index"
                ...
                <div class="item">
                  <div :class="{itemInner:true, 'isToday': today === day.date} ">
                    <span :class="{'show-formal':true,'isToday': today === day.date}">{{
                        today === day.date ? '今' : new Date(day.date).getDate()
                      }}</span>
...
export default {
  ...
  methods: {
    init () {
      console.log('planId init11111111', this.planId)
    ...
    findPatientInjectionCalendar (data) {
      findPatientInjectionCalendar(this.patientId, this.planId, data).then(res => {
        ...
        console.log('===', this.allInjectionDate)
    ...
    clickDayDetail (val) {
      ...
      this.$emit('changeSelectDate', {
        sendDate: this.sendDate,
        dateType: this.dateType,
        medicationDay: this.medicationDay
      })
    },
    editorCalendar (i) {
      if (i) {
