<template>
  <div class="main">
    ...
    <div style="display: flex; background: #fff; padding: 15px; margin-top: 1px">
      <div style="width: 50%">
        <div>
                ...
                <div v-for="(item, index) in getArray(i.jsonContent)" :key="index" style="padding: 5px 0">
                  <van-row v-if="getFormValue(item).type === 'text'">
                    ...
                    <van-col span="12" style="display: flex">
                      <div style="font-weight: 500; font-size: 14px; width: 90%; text-align: right"
                           :style="{ color: thisGetValueStatus(item, i.dataFeedBack) === 1 ? '#333333' : '#FF5A5A' }">
...
<script>
import {loadPatientBloodPressureTrendData, getMonitorList} from '@/api/formApi.js'
...
export default {
  ...
  methods: {
    ...
    addData () {
      this.$router.push({path: '/patient/monitor/pressureEditor', query: {planId: this.$route.query.planId}})
    },
    getDay (time) {
    ...
    loadMyBloodPressureTrendData () {
      loadPatientBloodPressureTrendData(localStorage.getItem('patientId')).then((res) => {
        if (res.code === 0) {
