<template>
  <div class="main">
    ...
    <div>
      ...
      <van-col span='24'
               style='text-align: center; position: absolute; bottom: 20px; width: 100%; height: 75px; padding: 10px 25px 0 25px'>
...
<script>
import {getGlucoseMessageId, getSugar, putSugarFormResult, sugarFormResult} from '@/api/plan.js'
import Vue from 'vue'
...
export default {
  ...
  methods: {
    ...
    getGlucoseMessageId () {
      getGlucoseMessageId(this.imMessageId).then(res => {
        if (res.data) {
    ...
    submitForm () {
      ...
      if (this.formData.id) {
        ...
        putSugarFormResult(params).then((res) => {
          this.submitStatus = false
          if (res.code === 0) {
            if (this.imMessageId) {
              this.$router.go(-1)
            } else {
              this.$router.replace('/patient/monitor/glucose')
            }
          }
        }).catch(() => {
          this.submitStatus = false
        ...
        sugarFormResult(params).then((res) => {
          this.submitStatus = false
          if (res.code === 0) {
            if (this.imMessageId) {
              this.$router.go(-1)
