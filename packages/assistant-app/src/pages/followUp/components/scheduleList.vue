<template>
  <div style="padding-top: 26px;
    ...
    <div v-if="planList.length === 0" style="display: flex; background: white; justify-content: center;height: 60vh">
      <van-empty :image="require('@/assets/my/no_data.png')" description="暂无记录"/>
    </div>
    <van-list
      ...
      <div class="list" v-for="(item,index) in planList" :key="item.id">
        <div style="display: flex;align-items: center;margin-bottom: 7px">
          <span><img :src="require('@/assets/my/panel-point.png')" style="width:13px;height: 13px;margin-right: 22px"
                     alt=""></span>
          <span style="color: #666666;font-size: 13px">执行计划第<span style="color: #3F86FF">{{item.planExecutionDay}}</span> 天</span>
        </div>
        <!-- 需要前端调整这里的样式 -->
        <div  v-if="planType === 2 || planType === 1" class="list-item" :style="{'border-left':index<planList.length-1?'1px solid #C1D8FF':'none' }">
          <div class="itembox1" >
            <div v-for="(dItem, dKey) in item.planDetails" :key="dKey" @click="goFrom(dItem)"
                 style="display: flex; justify-content: space-between;margin-bottom: 10px ">
              <div
                style="color:#333333;font-size: 15px; white-space: nowrap; width: 180px;overflow: hidden; text-overflow: ellipsis; margin-right: 15px;">
                {{ dItem.firstShowTitle }}
              </div>
        ...
        <!--  -->
        <div v-else class="list-item" v-for="(citem, key) in item.planDetails" :key='key'
...
<!--              <div style="color:#333333;font-size: 15px">{{ citem.firstShowTitle }}</div>-->
              <div v-for="(dItem, dKey) in citem.planDetailTimeDTOS" :key="dKey"
                   style="display: flex; justify-content: space-between; margin-top: 5px;margin-bottom: 8px" @click="goCMS(dItem)">
                <div
                  style="color:#333;font-size: 13px; white-space: nowrap; width: 180px;overflow: hidden; text-overflow: ellipsis; margin-right: 15px;">
          ...
          <!--血压血糖-->
          <div v-else-if="citem.planType === 2 || citem.planType === 1" class="itembox1">
              <div v-for="(dItem, dKey) in citem.planDetailTimeDTOS" :key="dKey"
                   style="display: flex; justify-content: space-between; margin-top: 5px" @click="goFrom(citem)">
                <div
                  style="color:#333333;font-size: 15px; white-space: nowrap; width: 180px;overflow: hidden; text-overflow: ellipsis; margin-right: 15px;">
                  {{ citem.firstShowTitle }}
                </div>
                <div style="display: flex;align-items: center">
                  <span style="color: #3F86FF;font-size: 13px">{{ getDate(dItem.planExecutionDate) }}</span>
                  <van-icon color="#C6C6C6" name="arrow"/>
...
import {appFindFollowPlanAll, appFindOtherPlan} from '@/api/followUp.js'
...
export default {
  ...
  methods: {
    updatePlanType (val) {
      this.planType = val
      console.log('updatePlanType planType', this.planType)
    },
    // 跳转到cms
    goCMS (item) {
      if (item.hrefUrl) {
        ...
        if (item.hrefUrl.startsWith('http')) {
          // 头部有http协议，不需要拼接
          this.$previewWeiXinCms(item.hrefUrl)
        } else {
          this.$previewWeiXinCms('http://' + item.hrefUrl)
        }
      } else {
        // 跳转到cms查看文章
    ...
    getDoctorFindFollowPlan () {
      appFindFollowPlanAll(this.tagIds, this.current).then(res => {
        if (res.code === 0) {
          if (this.current === 1) {
            this.$refs.loading.cancelLoading()
            this.planListData = res.data
            this.planList = res.data.planExecutionCycles
          } else {
            this.planList.push(...res.data.planExecutionCycles)
          }
          // 数据全部加载完成
          this.current++
          if (res.data.planExecutionCycles && res.data.planExecutionCycles.length === 0) {
            this.finished = true
          }
          this.loading = false
        }
    ...
    getDoctorFindOtherPlan () {
      appFindOtherPlan(this.planId, this.current).then(res => {
        if (res.code === 0) {
          if (this.current === 1) {
            this.$refs.loading.cancelLoading()
            this.planListData = res.data
