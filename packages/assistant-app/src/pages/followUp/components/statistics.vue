<template>
  <div style="min-height: 100vh;background: #f5f5f5">
    <van-sticky :offset-top="0">
      <headNavigation leftIcon="arrow-left" title="随访统计" @onBack="onBack"></headNavigation>
    </van-sticky>
    <!--详情-->
...
<script>
import {appQueryFollowCountDetail} from '@/api/followUp.js'
...
export default {
  ...
  methods: {
    onBack () {
      this.$router.replace({path: '/followUp'})
    },
    // 选择了某个计划
    selectPlan (item) {
      console.log(item)
    ...
    getDoctorQueryFollowCountDetail () {
      appQueryFollowCountDetail().then(res => {
        if (res.code === 0) {
          ...
          if (this.statisticsData.followTaskContentList && this.statisticsData.followTaskContentList.length > 0) {
            ...
            setTimeout(() => { this.planId = this.statisticsData.followTaskContentList[0].id }, 100)
          }
        }
      })
    },
