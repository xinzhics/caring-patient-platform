...
import {appQueryCountLearnPlanPush} from '@/api/followUp.js'
  ...
  methods: {
    ...
    goCMS (item) {
      ...
      if (item.cmsLink) {
        ...
        if (item.cmsLink.indexOf('caringcloud') > -1 || item.cmsLink.indexOf('caringsaas') > -1) {
          if (item.cmsLink.indexOf('wx/cms/show') > -1) {
            const id = item.cmsLink.substring(item.cmsLink.indexOf('id=') + 3)
            contentDetailNoContent(id).then(res => {
              if (res.code === 0) {
                const data = res.data
                if (!data) {
                  this.toLink(item.cmsLink)
                } else if (data.link) {
    ...
    getDoctorQueryCountLearnPlanPush () {
      appQueryCountLearnPlanPush().then(res => {
        if (res.code === 0) {
